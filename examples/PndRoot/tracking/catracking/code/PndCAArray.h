...
#define ALIHLTARRAY_STATIC_ASSERT(cond, msg) \
  ALIHLTARRAY_STATIC_ASSERT_NC(cond, msg);   \
  (void)Error_##msg
...
template <typename T>
struct TypeForAlignmentHelper<T, PndCAFullyCacheLineAligned> {
...
#define BOUNDS_CHECK(x, y)                              \
  if (PndCAInternal::ArrayBoundsCheck::IsInBounds(x)) { \
  } else                                                \
    return y
#endif
template <typename T, int alignment>
...
template <typename T>
class Allocator<T, PndCAFullyCacheLineAligned> {
 public:
...
class AlignedData {
 ...
 private:
  enum { Alignment = _alignment == PndCAFullyCacheLineAligned ? 128 : _alignment, PaddedSize = Size * sizeof(T) + Alignment };
...
class PndCAArray : public PndCAInternal::ArrayBase<T, Dim> {
 public:
  ...
  inline PndCAArray<Other, Dim> ReinterpretCast() const
  {
