...
class PndFTSCAClusterData {
 public:
   ...
   */
  PndFTSCAClusterData(const PndFTSCAGBHit *hits, int *offset, int numberOfClusters) { readEvent(hits, offset, numberOfClusters); }
  ...
  {
    assert(index < static_cast<int>(fData.size()));
