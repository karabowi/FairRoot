...
InitStatus PndTrkLegendreSecTask::Init()
{
  ...
  if (!fMvdPixelHitArray) {
    std::cout << "-W- PndTrkLegendreSecTask::Init: "
  ...
  if (fSecondary)
    legendre->SetUpLegendreHisto(180, 0, 180, 1000, -1., 1.);
  else
...
void PndTrkLegendreSecTask::Exec(Option_t *)
{
  ...
  for (int isec = 0; isec < 6; isec++) {
    if (stthitlist->GetNofHitsInSector(isec) == 0)
      continue;
    conformalhitlist = new PndTrkConformalHitList();
    if (fDisplayOn) {
      Refresh();
...
void PndTrkLegendreSecTask::FillLegendreHisto(Int_t mode)
{
  ...
  for (int ihit = 0; ihit < conformalhitlist->GetNofHits(); ihit++) {
    ...
    if (mode == 0 && chit->GetDetectorID() != FairRootManager::Instance()->GetBranchId(fSttBranch))
      continue;
...
void PndTrkLegendreSecTask::FillLegendreHisto(PndTrkCluster *cluster)
{
  ...
  for (int ihit = 0; ihit < conformalhitlist->GetNofHits(); ihit++) {
    PndTrkConformalHit *chit = conformalhitlist->GetHit(ihit);
    if (chit->GetDetectorID() == FairRootManager::Instance()->GetBranchId(fSttBranch))
      continue;
    PndTrkHit *hit = chit->GetHit();
    if (hit->IsUsed())
      continue;
