...
void PndTrkPlotMacros2::disegnaAssiXY(FILE *MACRO, double xmin, double xmax, double ymin, double ymax)
{
  // fine cambio_in_perl ;
...
// inizio cambio_in_perl ;
void PndTrkPlotMacros2::disegnaSciTilHit(int colorcode, // goes in the SetColor function of root;
...
void PndTrkPlotMacros2::DrawBiHexagonInMacro(Double_t vgap, FILE *MACRO, Double_t Ami, Double_t Ama, Short_t color, char *name)
{
  // fine cambio_in_perl ;
  Short_t iside;
...
void PndTrkPlotMacros2::DrawHexagonCircleInMacro(Double_t GAP, FILE *MACRO, Double_t ApotemaMin, Double_t Rma, Short_t color, char *name)
{
  // fine cambio_in_perl ;
  Short_t iside;
...
// inizio cambio_in_perl ;
void PndTrkPlotMacros2::SttInfoXYZParal(Vec<Double_t> *info, Short_t infopar, Double_t Oxx, Double_t Oyy, Double_t Rr, Double_t KAPPA, Double_t FI0, Short_t Charge,
                                        Double_t *Posiz)
{
  // fine cambio_in_perl ;
  ...
  PndTrkPlotMacros2_InputData In_Put)
{
  ...
  for (i = 0, k = -1; i < nTotalCandidates; i++) {
    ...
    for (j = 0; j < nSciTilHitsinTrack.at(i); j++) {
      ...
      intersect = GeometryCalculator.IntersectionSciTil_Circle(posizSciTil.at(ListSciTilHitsinTrack.at(i * MAXSCITILHITSINTRACK + j) * 3 + 0),
    ...
    for (j = 0; j < In_Put.nMCSciTilAlone[i]; j++) {
      intersect = GeometryCalculator.IntersectionSciTil_Circle(posizSciTil.at(In_Put.MCSciTilAloneList[i * nSciTilHits + j] * 3 + 0),
    ...
    if (nSttSkewHitsinTrack.at(i) + nMvdPixelHitsinTrack.at(i) + nMvdStripHitsinTrack.at(i) > 0 && doMcComparison) {
      ...
      WriteMacroSkewAssociatedHitswithMC_Degree(esseSciTil, esseSciTilAlone, In_Put,
                                                k, // questo si usa solo per il nome della Macro.
                                                i);
  ...
  WriteMacroParallelHitsGeneral(APOTEMAMAXINNERPARSTRAW, APOTEMAMAXSKEWSTRAW, APOTEMAMINOUTERPARSTRAW, APOTEMAMINSKEWSTRAW, BFIELD, CVEL, DIMENSIONSCITIL, doMcComparison,
                                ...
                                &posizSciTil, &primoangolo, &R, RSTRAWDETECTORMAX, APOTEMASTRAWDETECTORMIN, &sigmaXMvdPixel, &sigmaXMvdStrip, &sigmaYMvdPixel, &sigmaYMvdStrip,
  ...
  WriteMacroParallelHitsGeneralConformalwithMC(APOTEMAMAXINNERPARSTRAW, APOTEMAMAXSKEWSTRAW, APOTEMAMINOUTERPARSTRAW, APOTEMAMINSKEWSTRAW, BFIELD, CVEL, DIMENSIONSCITIL,
                                               ...
                                               &keepit, &FI0, &Ox, &Oy, &posizSciTil, &primoangolo, &R, RSTRAWDETECTORMAX, APOTEMASTRAWDETECTORMIN, &sigmaXMvdPixel,
  ...
  WriteMacroParallel_MvdHitsGeneralConformalwithMC(
    APOTEMAMAXINNERPARSTRAW, APOTEMAMAXSKEWSTRAW, APOTEMAMINOUTERPARSTRAW, APOTEMAMINSKEWSTRAW, BFIELD, CVEL, DIMENSIONSCITIL, doMcComparison, fMCTrackArray, nSttHit, &info,
    In_Put, IVOLTE, nMCTracks, nMvdPixelHit, nMvdStripHit, nSciTilHits, nTotalCandidates, &keepit, &FI0, &Ox, &Oy, &posizSciTil, &primoangolo, &R, RSTRAWDETECTORMAX,
    APOTEMASTRAWDETECTORMIN, &sigmaXMvdPixel, &sigmaXMvdStrip, &sigmaYMvdPixel, &sigmaYMvdStrip, &ultimoangolo, VERTICALGAP, &XMvdPixel, &XMvdStrip, &YMvdPixel, &YMvdStrip);
  ...
  WriteMacroAllHitsRestanti(APOTEMAMAXINNERPARSTRAW, APOTEMAMAXSKEWSTRAW, APOTEMAMINOUTERPARSTRAW, APOTEMAMINSKEWSTRAW, &InclusionListSciTil, &InclusionListStt, &info, IVOLTE,
                            ...
                            nTotalCandidates, &nTrackCandHit, &posizSciTil, RSTRAWDETECTORMAX, APOTEMASTRAWDETECTORMIN, VERTICALGAP, &XMvdPixel, &XMvdStrip, &YMvdPixel,
                            &YMvdStrip);
...
//  inizio cambio_in_perl ;
void PndTrkPlotMacros2::WriteMacroAllHitsRestanti(Double_t APOTEMAMAXINNERPARSTRAW, Double_t APOTEMAMAXSKEWSTRAW, Double_t APOTEMAMINOUTERPARSTRAW, Double_t APOTEMAMINSKEWSTRAW,
                                                  ...
                                                  Double_t APOTEMASTRAWDETECTORMIN, Double_t VERTICALGAP, Vec<Double_t> *XMvdPixel, Vec<Double_t> *XMvdStrip,
...
)
{
  ...
  DrawBiHexagonInMacro(VERTICALGAP, MACRO, APOTEMASTRAWDETECTORMIN, APOTEMAMAXINNERPARSTRAW,
                       4, // color code, 4= blue.
                       myname);
  //--------------
...
void PndTrkPlotMacros2::WriteMacroParallelHitsGeneral(Double_t APOTEMAMAXINNERPARSTRAW, Double_t APOTEMAMAXSKEWSTRAW, Double_t APOTEMAMINOUTERPARSTRAW,
                                                      ...
                                                      Double_t RSTRAWDETECTORMAX, Double_t APOTEMASTRAWDETECTORMIN, Vec<Double_t> *sigmaXMvdPixel, Vec<Double_t> *sigmaXMvdStrip,
...
void PndTrkPlotMacros2::WriteMacroParallelHitsGeneralConformalwithMC(
  ...
  Int_t Nhits, Vec<Double_t> *info, PndTrkPlotMacros2_InputData In_Put, int IVOLTE, Short_t nMCTracks, Short_t /*nMvdPixelHit*/, //[R.K. 9/2018] unused
  ...
  Vec<Double_t> *R, Double_t RSTRAWDETECTORMAX, Double_t APOTEMASTRAWDETECTORMIN, Vec<Double_t> * /*sigmaXMvdPixel*/,            //[R.K. 9/2018] unused
...
)
{
  ...
  fprintf(MACRO, "TEllipse* Griglia%d = new TEllipse(0.,0.,%f,%f,0.,360.);\nGriglia%d->SetLineColor(4);\nGriglia%d->Draw();\n", In_Put.NRDIVCONFORMAL, 1. / APOTEMASTRAWDETECTORMIN,
          1. / APOTEMASTRAWDETECTORMIN, In_Put.NRDIVCONFORMAL, In_Put.NRDIVCONFORMAL);
...
void PndTrkPlotMacros2::WriteMacroParallel_MvdHitsGeneralConformalwithMC(Double_t /*APOTEMAMAXINNERPARSTRAW*/,                         //[R.K. 9/2018] unused
                                                                         Double_t /*APOTEMAMAXSKEWSTRAW*/,                             //[R.K. 9/2018] unused
                                                                         Double_t /*APOTEMAMINOUTERPARSTRAW*/,                         //[R.K. 9/2018] unused
                                                                         Double_t /*APOTEMAMINSKEWSTRAW*/,                             //[R.K. 9/2018] unused
                                                                         Double_t BFIELD, Double_t CVEL, Double_t /*DIMENSIONSCITIL*/, //[R.K. 9/2018] unused
                                                                         bool doMcComparison, TClonesArray * /*fMCTrackArray*/,        //[R.K. 9/2018] unused
                                                                         Int_t Nhits, Vec<Double_t> *info, PndTrkPlotMacros2_InputData In_Put, int IVOLTE, Short_t nMCTracks,
                                                                         ...
                                                                         Vec<Double_t> *R, Double_t RSTRAWDETECTORMAX, Double_t APOTEMASTRAWDETECTORMIN,
...
void PndTrkPlotMacros2::WriteMacroSkewAssociatedHitswithMC(Double_t *ESSE,      // S of all associated SciTil hits to the present track;
                                                           ...
                                                           int iTrack)
{
  ...
  Double_t // xmin , xmax, ymin, ymax, //[R.K. 01/2017] unused variable?
           // dx, dy, diff, d1, d2, //[R.K. 01/2017] unused variable?
    deltaz,
    deltaS,                               // delta, deltax, deltay,  //[R.K. 01/2017] unused variable?
  ...
  for (iii = 0; iii < nSkewHitsinTrack[iTrack]; iii++) {
    ...
    if (!In_Put.InclusionListStt[i])
      continue;
    GeoCalculator.CalculateSandZ2(Oxx,               // input;
                                  Oyy,               // input;
                                  Rr,                // input;
    ...
    for (ii = 0; ii < 2; ii++) {
      if (auxZ[ii] < 999998.) {
        if (zmin > auxZ[ii] - auxZDrift[ii])
          zmin = auxZ[ii] - auxZDrift[ii];
        if (zmax < auxZ[ii] + auxZDrift[ii])
          zmax = auxZ[ii] + auxZDrift[ii];

        if (Smin > auxS[ii] - auxSDrift[ii])
          Smin = auxS[ii] - auxSDrift[ii];
        if (Smax < auxS[ii] + auxSDrift[ii])
          Smax = auxS[ii] + auxSDrift[ii];
  ...
  for (iii = 0; iii < nMCSkewAlone; iii++) {
    ...
    if (zmin > dis - auxZDrift[ii])
      zmin = dis - auxZDrift[ii];
    if (zmax < dis + auxZDrift[ii])
      zmax = dis + auxZDrift[ii];
  ...
  if (zmax >= zmin && Smax >= Smin) {
    ...
    for (iii = 0; iii < nSkewHitsinTrack[iTrack]; iii++) {
      ...
      for (ii = 0; ii < 2; ii++) {
        if (auxZ[ii] > 999998.)
          continue; // this solution was not physical;
...
void PndTrkPlotMacros2::WriteMacroSkewAssociatedHitswithMC_Degree(Double_t *ESSE,      // S of all associated SciTil hits to the present track;
                                                                  Double_t *ESSEalone, // S of 'Alone'  SciTil hits to the present track;
                                                                  PndTrkPlotMacros2_InputData In_Put,
                                                                  int iNome, // questo e' per il nome delle Macro solamente.
                                                                  int iTrack)
{
  ...
  if (zmax >= zmin && Smax >= Smin) {
    ...
    for (i = 0; i < nSciTilHitsinTrack[iTrack]; i++) {
      ...
      disegnaSciTilHit(icolore, // color code; the same as in SetColor of root;
                       DIMENSIONSCITIL, MACRO, posizSciTil[j * 3 + 2], TRA * ESSE[i], j,
                       1 // if 0 then SciTil draw in XY; if 1 then SciTil draw in SZ; else
                         // SciTil draw in UV.
      );
    }
    ...
    for (i = 0; i < In_Put.nMCSciTilAlone[iTrack]; i++) {
      j = In_Put.MCSciTilAloneList[iTrack * In_Put.nSciTilHits + i];
      disegnaSciTilHit(4, // color code; the same as in SetColor of root;
                       DIMENSIONSCITIL, MACRO, posizSciTil[j * 3 + 2], TRA * ESSEalone[i], j,
                       1 // if 0 then SciTil draw in XY; if 1 then SciTil draw in SZ; else
                         // SciTil draw in UV.
      );
    }
    ...
    for (iii = 0; iii < nSkewHitsinTrack[iTrack]; iii++) {
      ...
      for (ii = 0; ii < 2; ii++) {
        ...
        if (flaggo) {
          fprintf(MACRO, "TEllipse* spurioSkew%d_%d = new TEllipse(%f,%f,%f,%f,0.,360.,%f);\nspurioSkew%d_%d->SetFillStyle(0);\n", i, ii, auxZ[ii], TRA * auxS[ii], auxZDrift[ii],
                  TRA * auxSDrift[ii], 0., i, ii);
          fprintf(MACRO, "spurioSkew%d_%d->SetLineColor(2);\n", i, ii);
          fprintf(MACRO, "spurioSkew%d_%d->Draw();\n", i, ii);
        } else {
          fprintf(MACRO, "TEllipse* Skew%d_%d = new TEllipse(%f,%f,%f,%f,0.,360.,%f);\nSkew%d_%d->SetFillStyle(0);\n", i, ii, auxZ[ii], TRA * auxS[ii], auxZDrift[ii],
    ...
    for (iii = 0; iii < nMCSkewAlone; iii++) {
      ...
      fprintf(MACRO, "TEllipse* AloneSkew%d_%d = new TEllipse(%f,%f,%f,%f,0.,360.,%f);\nAloneSkew%d_%d->SetFillStyle(0);\n", i, ii, dis, TRA * auxS[ii], auxZDrift[ii],
              TRA * auxSDrift[ii], 0., i, ii);
      ...
      if (pMC) {
        ...
        if (fabs(carica) >= 0.1) {
          ...
          for (i = Nmin; i <= Nmax; i++) {
            ...
            z1 = (i * 2. * PI - FI0) / KAPPA;
            z2 = ((i + 1) * 2. * PI - FI0) / KAPPA;
            fprintf(MACRO, "TLine* MC%d_%d = new TLine(%f,%f,%f,%f);\nMC%d_%d->SetLineColor(3);\nMC%d_%d->Draw();\n", imc, i - Nmin, z1, 0., z2, TRA * 2. * PI, imc, i - Nmin, imc,
                    i - Nmin);
          } //  end of  for(i=Nmin; i<= Nmax;++)
...
void PndTrkPlotMacros2::WriteMacroSttParallelAssociatedHitsandMvdwithMC(PndTrkPlotMacros2_InputData In_Put, Double_t Oxx, Double_t Oyy, Double_t Rr, Double_t primoangolo,
                                                                        Double_t ultimoangolo, Short_t /*Nhits*/, //[R.K. 9/2018] unused
                                                                        int iTrack, int iNome, Short_t daSttTrackaMCTrack, Short_t nMvdPixelHitsAssociatedToSttTra,
...
void PndTrkPlotMacros2::WriteMacroSttParallel(PndTrkPlotMacros2_InputData In_Put)
{
  ...
  Double_t RSTRAWDETECTORMAX = In_Put.rstrawdetectormax, APOTEMASTRAWDETECTORMIN = In_Put.apotemastrawdetectormin, VERTICALGAP = In_Put.verticalgap,
           APOTEMAMAXINNERPARSTRAW = In_Put.apotemamaxinnerparstraw, APOTEMAMINSKEWSTRAW = In_Put.apotemaminskewstraw, APOTEMAMAXSKEWSTRAW = In_Put.apotemamaxskewstraw,
  ...
  for (int i = 0; i < In_Put.number_straws; i++) {
    ...
    fprintf(MACRO, "TEllipse* ParalTube%d = new TEllipse(%f,%f,%f,%f,0.,360.);\nParalTube%d->SetFillStyle(0);\nParalTube%d->Draw();\n", i + 1, center.X(), center.Y(), 0.5, 0.5,
            i + 1, i + 1);
  }
  //------------- hits paralleli spuri
