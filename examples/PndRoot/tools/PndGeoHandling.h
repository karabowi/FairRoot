 ...
 * @details
 *
 * To save memory not the full path of a volume is stored
 ...
 * This class helps converting the encrypted ID into the path
 * information and vice versa.
 * It needs the information of the GeoManager. Therefore one has to
...
class PndGeoHandling : public FairTask {
 public:
  ...
  bool VolumeIsSensitive(
    TString &path,
    std::vector<std::string> &listOfSensitives); ///< Checks if the path contains a substring which matches one of the given strings in listOfSensitives. If not false is returned.
