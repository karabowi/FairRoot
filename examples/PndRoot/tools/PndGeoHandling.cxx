...
PndGeoHandling *PndGeoHandling::fInstance = nullptr;

PndGeoHandling *PndGeoHandling::Instance()
{
  if (!fInstance) {
    LOG(info) << "Info in (PndGeoHandling::Instance): Making a new instance using the framework.";
...
PndGeoHandling::PndGeoHandling()
{
  ...
  if (run) {
    ...
    LOG(info) << "PndGeoHandling. No FairRun object found. If used in a macro take another constructor.";
  }
}

PndGeoHandling::PndGeoHandling(PndSensorNamePar *SensorNamePar) : fGeoMan(gGeoManager), fSensorNamePar(SensorNamePar)
{
  if (fInstance)
    return;
  fInstance = this;
...
{
  if (fRunId != 0)
    return; // SetParContainers was already called
  if (fSensorNamePar != nullptr)
    return;
  FairRun *run = FairRun::Instance();
  if (!run)
    Fatal("PndGeoHandling", "No FairRun object found.");
  fRtdb = run->GetRuntimeDb();
  if (!fRtdb)
    Fatal("PndGeoHandling", "No runtime database found.");
  fRunId = run->GetRunId();
  if (fRunId < 0)
    Error("PndGeoHandling", "No valid run ID? %i", fRunId);
  // if (!gGeoManager) GetGeoManager();
  fGeoMan = gGeoManager;
  if (!fGeoMan)
    Fatal("PndGeoHandling", "No gGeoManager found.");
  fSensorNamePar = (PndSensorNamePar *)(fRtdb->getContainer("PndSensorNamePar"));
  if (!fSensorNamePar)
    Fatal("PndGeoHandling", "No PndSensorNamePar parameters found.");
  fRtdb->initContainers(fRunId);
  FairTask::SetParContainers();
}

PndGeoHandling::PndGeoHandling(TString mcFile, TString parFile)
{
  if (fInstance)
    return;
  fInstance = this;
...
PndGeoHandling::PndGeoHandling(Int_t runId, TString parFile)
{
  if (fInstance)
    return;
  fInstance = this;
...
void PndGeoHandling::GetSensorNamePar()
{
  ...
  if (fVerbose > 1) {
    LOG(info) << "PndGeoHandling::GetSensorNamePar()";
    fRtdb->Print();
...
Int_t PndGeoHandling::GetShortID(TString path)
{
  ...
  if (fSensorNamePar != nullptr) {
    LOG(trace) << "PndGeoHandling::GetShortID for: " << path;
    return fSensorNamePar->SensorInList(&myPath);
  } else
    LOG(error) << "PndGeoHandling::GetShortID:" << path << " SensorNamePar is missing!";
  return -1;
}
...
TString PndGeoHandling::GetPath(Int_t shortID)
{
  if (fSensorNamePar != nullptr) {
    ...
    LOG(error) << " PndGeoHandling::GetPath(Int_t shortID): Missing SensorNamePar";
    abort();
  }
}
...
TMatrixD PndGeoHandling::MasterToLocalErrorsPath(const TMatrixD &master, const TString &path)
{
  ...
  result *= rot;
  if (fVerbose > 1) {
    LOG(info) << "PndGeoHandling::MasterToLocalErrorsPath: print matrices: master, rotation, result=R*M*R^T";
    master.Print();
    rot.Print();
    result.Print();
  }
...
TMatrixD PndGeoHandling::LocalToMasterErrorsPath(const TMatrixD &local, const TString &path)
{
  ...
  result *= rot;
  if (fVerbose > 1) {
    LOG(info) << "PndGeoHandling::LocalToMasterErrorsPath: print matrices: master, rotation, result=R^T*M*R";
    local.Print();
    rot.Print();
    result.Print();
  }
...
void PndGeoHandling::DiveDownToNode(TGeoNode *node)
{
  ...
  for (Int_t iNod = 0; iNod < currentNode->GetNdaughters(); iNod++) {
    fGeoMan->CdDown(iNod);
    DiveDownToNode(node);
    if (fGeoMan->GetCurrentNode() == node)
      return;
    fGeoMan->CdUp();
  }
}
  ...
  if (fSensorNamePar != nullptr) {
    ...
    if (VolumeIsSensitive(nodeName, listOfSensitives)) {
      ...
      if (sepString.size() > 0 && sepString[sepString.size() - 1].find("PartAss") == std::string::npos) {
        TObjString *sensName = new TObjString(fGeoMan->GetPath());
        Int_t id = GetIdForSensor(sensName->String());
        if (id > 0) {
          fSensorNamePar->AddSensorNameAndId(sensName, id);
        } else {
          fSensorNamePar->AddSensorName(sensName);
        }
      }
    }
    for (Int_t iNod = 0; iNod < currentNode->GetNdaughters(); iNod++) {
    ...
    LOG(error) << "PndGeoHandling::DiveDownToFillSensNamePar: fSensorNamePar does not exist!";
}
...
Int_t PndGeoHandling::GetIdForSensor(TString &sensName) const
{
  if (fCurrentIdMap) {
    Int_t id = -1;
    id = fCurrentIdMap->GetId(sensName);
    if (id > 0) {
      return id;
    }
  }
...
void PndGeoHandling::CreateUniqueSensorId(TString startName, std::vector<std::string> listOfSensitives)
{
  ...
  LOG_IF(info, fVerbose > 0) << "PndGeoHandling::CreateUniqueSensorId: StartNode: " << fGeoMan->GetPath();
  DiveDownToFillSensNamePar(listOfSensitives);
}
...
//___________________________________________________________________________________________
void PndGeoHandling::CreateUniqueSensorId(const TString &startName, const std::vector<std::string> &listOfSensitives, PndSensorNameIdMap *idmap)
{
  LOG(info) << "PndGeoHandling::CreateUniqueSensorId: Id map set. ";
  LOG_IF(warning, fGeoMan != gGeoManager) << "PndGeoHandling::CreateUniqueSensorId: Have to reset fGeoMan to gGeoManager!";
  if (fGeoMan != gGeoManager)
    fGeoMan = gGeoManager;
  fCurrentIdMap = idmap;
  fGeoMan->CdTop();
  DiveDownToNodeContainingString(startName);
  LOG(info) << "PndGeoHandling::CreateUniqueSensorId: StartNode: " << fGeoMan->GetPath();
  DiveDownToFillSensNamePar(listOfSensitives);
  fCurrentIdMap = nullptr;
}
