...
using namespace std;
// -------------   Default constructor  ----------------------------------
PndSolenoidMap::PndSolenoidMap() : PndFieldMap(), fRegionNo(0), fHemiX(0), fHemiY(0)
{
  fType = 2;
}
// ------------------------------------------------------------------------
...
PndSolenoidMap::PndSolenoidMap(const char *mapName, const char *fileType) : PndFieldMap(mapName, fileType), fRegionNo(0), fHemiX(0), fHemiY(0)
{
  fType = 2;
  fNumberOfRegions++;
...
PndSolenoidMap::PndSolenoidMap(PndMapPar *fieldPar) : PndFieldMap(), fRegionNo(fNumberOfRegions++), fHemiX(0), fHemiY(0)
{
  fType = 2;
  if (!fieldPar) {
    cerr << "-W- PndSolenoidMap::PndSolenoidMap: empty parameter container!" << endl;
    fName = "";
    fFileName = "";
    fType = -1;
  } else {
...
Bool_t PndSolenoidMap::IsInside(Double_t x, Double_t y, Double_t z, Int_t &ix, Int_t &iy, Int_t &iz, Double_t &dx, Double_t &dy, Double_t &dz)
{
  ...
  if (!(xl >= 0 && xl <= fXmax && yl >= 0 && yl <= fYmax && zl >= fZmin && zl <= fZmax)) {
    ix = iy = iz = 0;
    dx = dy = dz = 0.;
    return kFALSE;
  }
