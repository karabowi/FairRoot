...
PndFieldMap::PndFieldMap()
  : FairField(), fFileName(""), fScale(1.0), funit(10.0), fPosX(0), fPosY(0), fPosZ(0), fXmin(0), fXmax(0), fXstep(0), fYmin(0), fYmax(0), fYstep(0), fZmin(0), fZmax(0), fZstep(0),
    fNx(0), fNy(0), fNz(0), fBx(nullptr), fBy(nullptr), fBz(nullptr)
{
  SetName("");
  fType = 1;
}
...
PndFieldMap::PndFieldMap(const char *mapName, const char *fileType)
  : FairField(mapName), fFileName(TString("")), fScale(1.0), funit(10.0), fPosX(0), fPosY(0), fPosZ(0), fXmin(0), fXmax(0), fXstep(0), fYmin(0), fYmax(0), fYstep(0), fZmin(0),
    fZmax(0), fZstep(0), fNx(0), fNy(0), fNz(0), fBx(nullptr), fBy(nullptr), fBz(nullptr)
{
  SetName(mapName);
...
PndFieldMap::PndFieldMap(PndFieldPar *fieldPar)
  : FairField(), fFileName(TString("")), fScale(1.0), funit(10.0), fPosX(0), fPosY(0), fPosZ(0), fXmin(0), fXmax(0), fXstep(0), fYmin(0), fYmax(0), fYstep(0), fZmin(0), fZmax(0),
    fZstep(0), fNx(0), fNy(0), fNz(0), fBx(nullptr), fBy(nullptr), fBz(nullptr)
{
  fType = 1;
  if (!fieldPar) {
...
PndFieldMap::PndFieldMap(const PndFieldMap &L)
  : FairField(), fFileName(L.fFileName), fScale(L.fScale), funit(L.fScale), fPosX(L.fPosX), fPosY(L.fPosY), fPosZ(L.fPosZ), fXmin(L.fXmin), fXmax(L.fXmax), fXstep(L.fXstep),
    fYmin(L.fYmin), fYmax(L.fYmax), fYstep(L.fYstep), fZmin(L.fZmin), fZmax(L.fZmax), fZstep(L.fZstep), fNx(L.fNx), fNy(L.fNy), fNz(L.fNz), fBx(L.fBx), fBy(L.fBy), fBz(L.fBz)
{
  fType = L.fType;
...
Bool_t PndFieldMap::IsInside(Double_t x, Double_t y, Double_t z, Int_t &ix, Int_t &iy, Int_t &iz, Double_t &dx, Double_t &dy, Double_t &dz)
{
...
void PndFieldMap::WriteAsciiFile(const char *fileName)
{
  ...
  cout << "-I- PndFieldMap: " << fNx * fNy * fNz << " entries to write... " << setw(3) << 0 << " % ";
  Int_t index = 0;
  div_t modul;
...
void PndFieldMap::WriteRootFile(const char *fileName, const char *mapName)
{
...
void PndFieldMap::Print()
{
  ...
  cout << "----  z = " << setw(4) << fZmin << " to " << setw(4) << fZmax << " cm, " << fNz << " grid points, dz = " << fZstep << " cm" << endl;
  cout << endl;
  ...
  cout << "----" << endl;
  cout << "----  Field at origin is ( " << setw(6) << bx << ", " << setw(6) << by << ", " << setw(6) << bz << ") kG" << endl;
  cout << "======================================================" << endl;
}
// ------------------------------------------------------------------------
...
void PndFieldMap::ReadAsciiFile(const char *fileName)
{
  ...
  // Open file
  LOG(info) << " PndFieldMap: Reading field map from ASCII file " << fileName;
  ...
  if (type == "Trans")
    iType = 4;
  if (fType != iType) {
    LOG(error) << " PndFieldMap::ReadAsciiFile: Incompatible map types!";
  ...
  if (unit == "G")
    ...
    LOG(error) << " FieldMap::ReadAsciiFile: No units!";
  ...
  cout << right;
  Int_t nTot = fNx * fNy * fNz;
  cout << "-I- PndFieldMap: " << nTot << " entries to read... " << setw(3) << 0 << " % ";
  Int_t index = 0;
  div_t modul;
  Int_t iDiv = TMath::Nint(nTot / 100.);
  for (Int_t ix = 0; ix < fNx; ix++) {
    for (Int_t iy = 0; iy < fNy; iy++) {
      for (Int_t iz = 0; iz < fNz; iz++) {
        if (!mapFile.good())
          cerr << "-E- PndFieldMap::ReadAsciiFile: "
...
void PndFieldMap::ReadRootFile(const char *fileName, const char *mapName)
{
  ...
  PndFieldMapData *data = nullptr;
  file->GetObject(mapName, data);
  if (!data) {
    LOG(error) << "PndFieldMap::ReadRootFile: data object " << mapName << " not found in file! " << fileName;
    exit(-1);
  }
