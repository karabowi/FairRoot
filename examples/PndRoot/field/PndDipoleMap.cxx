...
using namespace std;
// -------------   Default constructor  ----------------------------------
PndDipoleMap::PndDipoleMap() : PndFieldMap(), fRegionNo(0), fHemiX(0), fHemiY(0), fBeamMom(0.)
{
  fType = 3;
}
// ------------------------------------------------------------------------
...
PndDipoleMap::PndDipoleMap(const char *mapName, const char *fileType, Double_t BeamMom) : PndFieldMap(mapName, fileType), fRegionNo(0), fHemiX(0), fHemiY(0), fBeamMom(BeamMom)
{
  fType = 3;
  TString Suffix = "";
  ...
  if (fBeamMom < 3)
    Suffix = ".0150";
  else if (fBeamMom < 6.0 && fBeamMom >= 3.0)
    Suffix = ".0406";
  else if (fBeamMom < 10.0 && fBeamMom >= 6.0)
    Suffix = ".0890";
  else if (fBeamMom < 13.0 && fBeamMom >= 10.0)
    Suffix = ".1191";
  else if (fBeamMom >= 13.0)
    Suffix = ".1500";
...
PndDipoleMap::PndDipoleMap(PndMapPar *fieldPar) : PndFieldMap(), fRegionNo(0), fHemiX(0), fHemiY(0), fBeamMom(0)
{
  ...
  fBx = fBy = fBz = nullptr;
  if (!fieldPar) {
    cerr << "-W- PndDipoleMap::PndDipoleMap: empty parameter container!" << endl;
    fName = "";
    fFileName = "";
    fType = 1;
  } else {
...
Bool_t PndDipoleMap::IsInside(Double_t x, Double_t y, Double_t z, Int_t &ix, Int_t &iy, Int_t &iz, Double_t &dx, Double_t &dy, Double_t &dz)
{
