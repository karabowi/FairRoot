...
// -----   Default constructor   -------------------------------------------
PndConstField::PndConstField() : fXmin(0), fXmax(0), fYmin(0), fYmax(0), fZmin(0), fZmax(0), fBx(0), fBy(0), fBz(0)
{
  fType = 0;
}
// -------------------------------------------------------------------------
...
PndConstField::PndConstField(const char *name, Double_t xMin, Double_t xMax, Double_t yMin, Double_t yMax, Double_t zMin, Double_t zMax, Double_t bX, Double_t bY, Double_t bZ)
  : FairField(name), fXmin(xMin), fXmax(xMax), fYmin(yMin), fYmax(yMax), fZmin(zMin), fZmax(zMax), fBx(bX), fBy(bY), fBz(bZ)
...
PndConstField::PndConstField(PndConstPar *fieldPar) : FairField(), fXmin(0), fXmax(0), fYmin(0), fYmax(0), fZmin(0), fZmax(0), fBx(0), fBy(0), fBz(0)
{
  if (!fieldPar) {
...
// -----   Set field region   ----------------------------------------------
void PndConstField::SetFieldRegion(Double_t xMin, Double_t xMax, Double_t yMin, Double_t yMax, Double_t zMin, Double_t zMax)
{
  fXmin = xMin;
  fXmax = xMax;
  fYmin = yMin;
...
void PndConstField::GetBxyz(const Double_t point[3], Double_t *bField)
{
  ...
  if (point[0] < fXmin || point[0] > fXmax || point[1] < fYmin || point[1] > fYmax || point[2] < fZmin || point[2] > fZmax) {
    bField[0] = 0;
    bField[1] = 0;
    bField[2] = 0;
  } else {
