...
class PndSdsDigiPixelMCInfo : public PndSdsDigiPixel {
  ...
  {
    out << "PndSdsDigiPixelMCInfo in: " << digi.GetSensorID() << " FE: " << digi.GetFE() << " Col/Row " << digi.GetPixelColumn() << "/" << digi.GetPixelRow() << " charge "
        << digi.GetCharge() << " e"
        << " timestamp " << digi.GetTimeStamp() << ", from Point(s) ";
    std::vector<Int_t> indices = digi.GetIndices();
    for (unsigned int i = 0; i < indices.size(); i++) {
      std::cout << indices[i] << "  ";
    }

    out << " MCCharge: " << digi.GetMCCharge() << " AddNoise: " << digi.GetAddNoise() << " TimeWalk: " << digi.GetTimeWalk()
        << " TimeWalkCorrection: " << digi.GetTimeWalkCorrection();
    std::cout << std::endl;
 ...
 public:
  ...
  PndSdsDigiPixelMCInfo(std::vector<Int_t> index, Int_t detID, Int_t sensorID, Int_t fe, Int_t col, Int_t row, Double_t totcharge, Double_t timeStamp, Double_t chargewonoise,
                        Double_t addnoise, Double_t timewalk, Double_t timewalkcorrection, Double_t tof, Double_t digicharge);
