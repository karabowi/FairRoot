...
class PndSdsPixel {
 public:
  PndSdsPixel() : fFe(-1), fCol(-1), fRow(-1), fCharge(-1.0), fSensorID(-1), fMCIndex(), fAddNoise(0.){};

  PndSdsPixel(Int_t sensorID, Int_t fe, Int_t col, Int_t row, Double_t charge, int index = -1)
    : fFe(fe), fCol(col), fRow(row), fCharge(charge), fSensorID(sensorID), fMCIndex(), fAddNoise(0.)
  {
    if (index >= 0)
  ...
  {
    out << "Detector: " << pixel.GetSensorID() << " FE: " << pixel.GetFE() << " Pixel (C/R): " << pixel.GetCol() << " " << pixel.GetRow() << " Charge: " << pixel.GetCharge()
        << " from MCHit: " << pixel.GetFirstMCIndex();
    return out;
  };
