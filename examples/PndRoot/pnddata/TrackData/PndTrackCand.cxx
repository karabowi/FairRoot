...
void PndTrackCand::AddHit(UInt_t detId, UInt_t hitId, Double_t rho)
{
...
void PndTrackCand::CalcTimeStamp()
{
  ...
  for (int i = 0; i < GetNLinks(); i++) {
    ...
    if (type > -1) {
      ...
      if (branchName.Contains("Hit") == kFALSE)
        continue;
...
{
  if (rhs.fHitId.size() != fHitId.size())
    return false;
  for (unsigned int i = 0; i < fHitId.size(); ++i) {
    if (fHitId.at(i) != rhs.fHitId.at(i))
      return false;
  }
  return true;
}
