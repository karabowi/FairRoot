...
class PndStack : public FairGenericStack {
 ...
 public:
   ...
   **/
  virtual void PushTrack(Int_t toBeDone, Int_t parentID, Int_t pdgCode, Double_t px, Double_t py, Double_t pz, Double_t e, Double_t vx, Double_t vy, Double_t vz, Double_t time,
  ...
  virtual void Print(Int_t iVerbose = 0) const;
    ...
    {
        LOG(info) << "Clone PANDA Stack";
        auto clonedStack = new PndStack();
   ...
   **/
  void AddPoint(PndDetectorId iDet);

  /** Increment number of points for an arbitrary track in a given detector
   *@param iDet    Detector unique identifier
   *@param iTrack  Track number
   **/
  void AddPoint(PndDetectorId iDet, Int_t iTrack);
  ...
  TClonesArray *GetListOfParticles() { return fParticles; }
    ...
    void AddGeoPoint(int trID, double trX, double trY, double trZ, double moX, double moY, double moZ) override;
 ...
 private:
  ...
  /** Array of PndMCTracks containg the tracks written to the output **/
    TClonesArray *fTracks {new TClonesArray("PndMCTrack", 100)};
    std::vector<PndMCTrack>* fTrackVector{new std::vector<PndMCTrack>};
    std::vector<int>* fGeoTrIdVector{new std::vector<int>};
    std::vector<double>* fGeoTrXVector{new std::vector<double>};
    std::vector<double>* fGeoTrYVector{new std::vector<double>};
    std::vector<double>* fGeoTrZVector{new std::vector<double>};
    std::vector<double>* fGeoTrPxVector{new std::vector<double>};
    std::vector<double>* fGeoTrPyVector{new std::vector<double>};
    std::vector<double>* fGeoTrPzVector{new std::vector<double>};
