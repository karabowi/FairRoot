...
class PndOtHit : public FairHit {
 ...
 public:
  ...
  // THIS ONE!
  PndOtHit(Int_t detID, Int_t tubeID, Int_t chamberID, Int_t layerID, Int_t skew, Int_t mcindex, TVector3 &pos, TVector3 &dpos, Double_t p, Double_t isochrone,
  ...
  {
    PndOtHit *myDigi = dynamic_cast<PndOtHit *>(data);
    if (myDigi != nullptr) {
  ...
  friend std::ostream &operator<<(std::ostream &out, PndOtHit &digi)
  {
