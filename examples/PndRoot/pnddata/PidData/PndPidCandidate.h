...
class PndPidCandidate : public FairTimeStamp {
 ...
 public:
  ...
  PndTrack::TrackType GetTrackType() const { return fTrackType; }
  ...
  Float_t GetEmcTimeStamp() const { return fEmcTimeStamp; }
  ...
  Int_t GetPidHypo() const { return fPidHyp; }
  ...
  void SetTrackType(PndTrack::TrackType type) { fTrackType = type; }

  void SetHelixParams(Double_t *par)
  {
    if (par != 0)
      for (int i = 0; i < 5; i++)
        fParams[i] = (Float_t)par[i];
  }
  void SetHelixCov(Double_t *cov)
  {
    if (cov != 0)
      for (int i = 0; i < 15; i++)
        fCov[i] = (Float_t)cov[i];
  }
  ...
  void SetPidHypo(int hyp) { fPidHyp = hyp; }
  void SetDefault();
 ...
 protected:
  ...
  Float_t fFirstHitX, fFirstHitY, fFirstHitZ;

  Float_t fLastHitX, fLastHitY, fLastHitZ;

  Int_t fMcIndex;     // MC truth index
  Int_t fTrackIndex;  // PndTrack index
  Int_t fTrackBranch; // index of PndTrack TClonesArray
  PndTrack::TrackType fTrackType = PndTrack::TrackType::kUndefined; // type of PndTrack (forward, barrel, combined)
