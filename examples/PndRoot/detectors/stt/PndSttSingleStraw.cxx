...
// ==============================================================
void PndSttSingleStraw::PutWireXYZ(Double_t w1, Double_t w2, Double_t w3, Double_t w4, Double_t w5, Double_t w6)
{
  ...
  for (Int_t k = 0; k < ido; k++) {
    ...
    if (pSTP < 1.9) {
      ...
      if (Field == 1 && CO2Perc < 0.15) {
        etime = -1.569123e-03 + 2.228707e-01 * dst - 2.100147e-01 * pow(dst, 2) + 1.240429 * pow(dst, 3) - 5.929662e-01 * pow(dst, 4);
        etime = etime * 1000.; // from micros to ns
      } else if (Field == 1 && CO2Perc > 0.15) {
        etime = -3.022419e-04 + 1.490671e-01 * dst - 1.947376e-01 * pow(dst, 2) + 2.764081e+00 * pow(dst, 3) - 2.129739e+00 * pow(dst, 4);
        etime = etime * 1000.; // from micros to ns
      }
      // no magnetic field 90/10
      else {
        etime = -1.472305e-03 + 2.139223e-01 * dst - 5.835399e-02 * pow(dst, 2) - 6.727544e-02 * pow(dst, 3) + 6.334636e-01 * pow(dst, 4);
        etime = etime * 1000.; // from micros to ns
      }
    }
...
Int_t PndSttSingleStraw::StrawTime()
{
  ...
  if (flag1 == 1 && flag2 == 1)
    PulseTime = PulseT.at(ind) - OffT; // time in ns;
...
Double_t PndSttSingleStraw::TimnsToDiscm(Double_t time)
{
  ...
  if (pSTP < 1.9) {
    ...
    if (CO2Perc < 0.15) {
      ...
      drift = 2.498729e-02 - 2.788627e-03 * time + 3.997619e-03 * pow(time, 2) - 1.289555e-04 * pow(time, 3) + 2.101670e-06 * pow(time, 4) - 1.959622e-08 * pow(time, 5) +
              1.053198e-10 * pow(time, 6) - 3.030411e-13 * pow(time, 7) + 3.609173e-16 * pow(time, 8);
    } else {
      // fixed threshold 80/20
      drift = -4.843174e-02 + 4.859828e-03 * time + 4.593485e-03 * pow(time, 2) - 1.677031e-04 * pow(time, 3) + 2.955270e-06 * pow(time, 4) - 2.996812e-08 * pow(time, 5) +
              1.832841e-10 * pow(time, 6) - 6.678677e-13 * pow(time, 7) + 1.335729e-15 * pow(time, 8) - 1.128932e-18 * pow(time, 9);
    }
  }
...
Double_t PndSttSingleStraw::FastRec(Double_t TrueDcm, Int_t Flag)
{
  ...
  if (pSTP < 1.9) {
    ...
    // radius > 0.4 cm
      ...
      //      if(TrueDcm < 0.48){
      resmic = 1.53656e+02 - 5.07749e+03 * TrueDcm + 1.73707e+05 * pow(TrueDcm, 2) - 2.72285e+06 * pow(TrueDcm, 3) + 2.28719e+07 * pow(TrueDcm, 4) - 1.12921e+08 * pow(TrueDcm, 5) +
               3.39427e+08 * pow(TrueDcm, 6) - 6.12741e+08 * pow(TrueDcm, 7) + 6.12041e+08 * pow(TrueDcm, 8) - 2.60444e+08 * pow(TrueDcm, 9);
      //       }
      //       else resmic = 72.;
    }
  }
  ...
  // 2 atm pressure radius 5 cm
    ...
    if (Flag == 0) {
      ...
      //       if(TrueDcm < 0.48){
      resmic = +1.48048e+02 - 3.35951e+02 * TrueDcm - 1.87575e+03 * pow(TrueDcm, 2) + 1.92910e+04 * pow(TrueDcm, 3) - 6.90036e+04 * pow(TrueDcm, 4) +
