...
class Ex9TMessage2 : public TMessage {
 public:
  Ex9TMessage2(void *buf, Int_t len) : TMessage(buf, len) { ResetBit(kIsOwner); }
};
...
PndMvdMQTaskProcessor::PndMvdMQTaskProcessor()
  : FairMQDevice(), fNewRunId(1), fCurrentRunId(-1), fInput(nullptr), fOutput(nullptr), fEventHeader(nullptr), fFairTask(nullptr), fGeoPar(nullptr), fParCList(nullptr)
{
}
...
FairParGenericSet *PndMvdMQTaskProcessor::UpdateParameter(FairParGenericSet *thisPar)
{
  ...
  if (Send(req, "param") > 0) {
    if (Receive(rep, "param") > 0) {
      ...
      LOG(warning) << "Received parameter" << paramName << " from the server (" << thisPar << ")";
      return thisPar;
    }
  }
  return nullptr;
