...
PndFts::PndFts()
  : fTrackID(0), fVolumeID(0), fPos(0, 0, 0, 0), fPosIn(0, 0, 0, 0), fPosOut(0, 0, 0, 0), fPosInLocal(0, 0, 0, 0), fPosOutLocal(0, 0, 0, 0), fMomIn(0, 0, 0, 0),
    fMomOut(0, 0, 0, 0), fTime(0), fLength(0), fELoss(0), fMass(0), fIsInitialized(kFALSE), fPosIndex(0), fpostot(0, 0, 0, 0), fpostotin(0, 0, 0, 0),
...
PndFts::PndFts(const char *name, Bool_t active)
  : FairDetector(name, active), fTrackID(0), fVolumeID(0), fPos(0, 0, 0, 0), fPosIn(0, 0, 0, 0), fPosOut(0, 0, 0, 0), fPosInLocal(0, 0, 0, 0), fPosOutLocal(0, 0, 0, 0),
    fMomIn(0, 0, 0, 0), fMomOut(0, 0, 0, 0), fTime(0), fLength(0), fELoss(0), fMass(0), fIsInitialized(kFALSE), fPosIndex(0), fpostot(0, 0, 0, 0),
...
PndFts::PndFts(const PndFts& rhs)
    : FairDetector(rhs), fTrackID(0), fVolumeID(0), fPos(0, 0, 0, 0), fPosIn(0, 0, 0, 0), fPosOut(0, 0, 0, 0), fPosInLocal(0, 0, 0, 0), fPosOutLocal(0, 0, 0, 0),
    fMomIn(0, 0, 0, 0), fMomOut(0, 0, 0, 0), fTime(0), fLength(0), fELoss(0), fMass(0), fIsInitialized(kFALSE), fPosIndex(0), fpostot(0, 0, 0, 0),
    fpostotin(0, 0, 0, 0), fpostotout(0, 0, 0, 0), fPassNodes(), valid(kFALSE), fGeoType(0)
{
}
...
Bool_t PndFts::ProcessHits(FairVolume *vol)
{
  ...
  if (gMC->TrackCharge() != 0.) {
    ...
    if (gMC->IsTrackExiting() && valid == kTRUE) {
      ...
      if (volumename.Contains("fts01"))
        basename = "fts01tube";
      if (volumename.Contains("fts02"))
        basename = "fts02tube";
      if (volumename.Contains("fts31"))
        basename = "fts31tube";
      if (volumename.Contains("fts32"))
        basename = "fts32tube";
      if (volumename.Contains("fts33"))
        basename = "fts33tube";
      if (volumename.Contains("fts34"))
        basename = "fts34tube";
      if (volumename.Contains("fts35"))
        basename = "fts35tube";
      if (volumename.Contains("fts36"))
        basename = "fts36tube";
      if (volumename.Contains("fts37"))
        basename = "fts37tube";
      if (volumename.Contains("fts38"))
        basename = "fts38tube";
      if (volumename.Contains("fts05"))
        basename = "fts05tube";
      if (volumename.Contains("fts06"))
        basename = "fts06tube";

      fullName = basename + hashmark + number;
  
      FairGeoNode *volnode = dynamic_cast<FairGeoNode *>(fPassNodes->FindObject(fullName.c_str()));
      ...
      if (fTrackID != 0) {
        if (fVerboseLevel > 2)
          std::cout << "befor Mapper------" << std::endl;
      }
...
{
  FairRootManager::Instance()->RegisterAny("FTSPoint", fPointVector, kTRUE);
}
// -------------------------------------------------------------------------
...
void PndFts::AddHit(Int_t trackID, Int_t detID, Int_t tubeID, Int_t chamberID, Int_t layerID, TVector3 pos, TVector3 posInLocal, TVector3 posOutLocal, TVector3 momIn,
                            TVector3 momOut, Double_t time, Double_t length, Double_t eLoss, Double_t mass) // da cancellare postot
{
    fPointVector->push_back(PndFtsPoint(trackID, detID, tubeID, chamberID, layerID, pos, posInLocal, posOutLocal, momIn, momOut, time, length, eLoss, mass));

  fPointVector->back().SetTubeID(tubeID);
  fPointVector->back().SetChamberID(chamberID);
  fPointVector->back().SetLayerID(layerID);

}
// -------------------------------------------------------------------------

Bool_t PndFts::IsSensitive(const std::string& name)
{
...
FairModule* PndFts::CloneModule() const
{
    PndFts* newDet = new PndFts(*this);
    newDet->InitOnWorker();
    return newDet;
}
