...
Double_t PndHypGePeakFitFunction::PeakFuncFreeSkewedPosition(Double_t *x, Double_t *par)
{
  ...
  //				* TMath::Erfc( ((x[0] - par[7]) /(TMath::Sqrt(2)*par[8]) ) + par[8]/TMath::Sqrt(2)/par[6]); //par[5]: amplitude skewed gaus, par[6]: beta,  par[7]: x0_s, par[8]:
  // sigma_s
...
{
  Double_t SkewedGausian = par[6] * TMath::Exp((x[0] - par[1]) / par[7]) *
...
{
  Double_t SkewedGausian =
    par[6] * TMath::Exp((x[0] - par[8]) / par[7]) *
...
{
  Double_t NewSkewedGausian = par[6] * TMath::Exp(-1 / 2 * pow((x[0] - par[8]) / par[9], 2)) * (1 + TMath::Erf(1 / sqrt(2) * par[7] * (x[0] - par[8]) / par[9]));
...
{
  Double_t value = par[3] * par[0] / 2 * TMath::Exp(-par[0] / 2 * (2 * par[1] + par[0] * par[2] * par[2] - 2 * x[0])) *
                     TMath::Erfc(-(par[1] + par[0] * par[2] * par[2] - x[0]) / TMath::Sqrt(2) / par[2]) +
                   par[4] * TMath::Gaus(x[0], par[5], par[6]) + par[7];
  return value;
}
