...
// -----   Default constructor   -------------------------------------------
PndHypFullIdealAna::PndHypFullIdealAna() : FairTask("Panda HypFullAna	 Task") {}
// -------------------------------------------------------------------------
...
InitStatus PndHypFullIdealAna::Init()
{
  ...
  LOG(info) << " PndHypFullAna  : Intialization successfull";
...
void PndHypFullIdealAna::Exec(Option_t *)
{
  ...
  for (int c = 0; c < chargedCands.GetLength(); c++) {
    ...
    for (int d = 0; d < xiCands.GetLength(); d++) {
      if (chargedCands[c].Overlaps(xiCands[d]))
        notinlist = false;
      if (notinlist)
        nonOvCands.Append(chargedCands[c]);
    }
  }
...
void PndHypFullIdealAna::Finish(TString cat)
{
  ...
  if (sink->GetSinkType() == kFILESINK) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      outfile->WriteTObject(spectra[i]);
      delete spectra[i];
      spectra[i] = nullptr;
    }
  }
