...
PndLmdDigiQTask::PndLmdDigiQTask(TString digiBranch) : FairTask("Digis Quality Task for PANDA Lmd"), fEventNr(0), lmd_geo_helper(PndLmdGeometryHelper::getInstance())
{
  fWriteAllMC = false;
...
InitStatus PndLmdDigiQTask::Init()
{
  ...
  if (!fMCHits) {
    std::cout << "-W- PndLmdDigiQTask::Init: "
              << "No LMDPoint"
              << " array!" << std::endl;
    return kERROR;
  }
