...
InitStatus PndLmdTrkQTask::Init()
{
  ...
  if (!fRecTracks) {
    std::cout << "-W- PndLmdTrkQTask::Init: "
              << "No " << fTrackBranchName << " array!" << std::endl;
    return kERROR;
  }
...
void PndLmdTrkQTask::Exec(Option_t *)
{
  ...
  if (fVerbose > 0)
    cout << "%%%%%! Event #" << fEventNr << " has " << nParticles << " true particles, "
         << " out of it " << nRecHits << " hits, " << nTrkCandidates << " trk-cands, " << numTrk << " tracks and " << nGeaneTrks << " geane Trks!" << endl;
  ...
  for (int irec = 0; irec < nRecHits; irec++) {
    ...
    if (mcreftop > 0 && mcrefbot > 0) {
      if (MCtrkidtop == MCtrkidbot) {
        ...
        if (fVerbose > 7)
          cout << "  REChit No." << irec << "contain MCid: " << MCtrkidtop << ", " << MCtrkidbot << " !" << endl;
      }
    }
    // if(fVerbose>7)
  ...
  for (Int_t iN = 0; iN < nGeaneTrks; iN++) {
    ...
    for (Int_t jN = (iN + 1); jN < nGeaneTrks; jN++) {
      ...
      if (mc_i == mc_j) {
        RECassigMC[iN]++;
        if (fVerbose > 3)
          cout << "GHOST?!: rec.trks #" << iN << " and #" << jN << "were assigned to one MC trk =|" << endl;
      }
    }
  }
  ...
  for (Int_t iN = 0; iN < nGeaneTrks; iN++) { // ghost/good trk determination
    ...
    //    tRECMCtrks->Fill();
    PndLmdTrackQ *trkqlmd = new ((*fTrackQ)[fTrackQ->GetEntriesFast()]) PndLmdTrackQ(glMommc);
    ...
    for (int imc = 0; imc < nParticles; imc++) { // MC trks
      ...
      if (missTrk) {
        if (MCtksREChits[imc] > minHits) { // missed during trk-search
          if (fVerbose > 7) {
            cout << " --- MCtrk#" << imc << " was defined as MISSED during trk-search (#MChits=" << MCtksREChits[imc] << " with limit>" << minHits << ")" << endl;
            if (MCDoubleHits[imc] > 0)
              cout << "         NB:this MCtrk contains " << MCDoubleHits[imc] << " double hits!" << endl;
          }
          trkQ = -1;
          nMCmissedTrkSearch++;
          ...
          if (MCtksREChits[imc] > 0) { // if MCtksREChits[imc]==0 trk is out of measurment range
            ...
            trkQ = -2;
            if (fVerbose > 7)
              cout << " --- MCtrk#" << imc << " was defined as MISSED due to little amount of hits (#MChits=" << MCtksREChits[imc] << " with limit>" << minHits << ")" << endl;
            int hitArr = -1;
