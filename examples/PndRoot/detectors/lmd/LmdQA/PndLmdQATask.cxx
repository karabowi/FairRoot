...
PndLmdQATask::PndLmdQATask(TString mcHitBranch, TString mcTrkBranch, TString clusterBranch, TString digiBrunch, TString hitBranch, TString trkCandBranch, TString trackBranch,
                           TString geaneBranch, TString outFile)
...
InitStatus PndLmdQATask::Init()
{
  ...
  LOG(info) << " PndLmdQATask: Initialisation successfull";
  return kSUCCESS;
}
...
bool PndLmdQATask::HitReco()
{
  ...
  for (Int_t i = 0; i < nRecHits; i++) {
    ...
    TVector3 mcvec(mc->GetX() + mc->GetXOut(), mc->GetY() + mc->GetYOut(), mc->GetZ() + mc->GetZOut());
    mcvec *= 0.5;

    //      TVector3 mcvec(mc->GetXOut(),mc->GetYOut(),mc->GetZOut());
    TVector3 dvec(recovec - mcvec);
    // if(1e4*(mc->GetZOut()-mc->GetZ())<0)
    // 	cout<<"For MCtrk#"<<MCidTOP<<" mc.Z() = "<<0.5*(mc->GetZ()+mc->GetZOut())<<" diff rec- mc [mkm] (x,y,z)  = ("<<1e4*dvec.X()<<", "<<1e4*dvec.Y()<<",
    // "<<1e4*dvec.Z()<<")"<<endl;
...
void PndLmdQATask::ResoAndPulls()
{
  ...
  for (Int_t iN = 0; iN < nGeaneTrks; iN++) { // loop over all reconstructed trks
    ...
    Double_t err_phi =
      (-((MomRecPCA.Y() * fDPPCA / cLmPCA) / pow(fPPCA, 2)) + (errMomRecPCA.Y() / cLmPCA) / fPPCA + (MomRecPCA.Y() * err_lyambda * TMath::Tan(fLmPCA) / cLmPCA) / fPPCA) /
    ...
    Double_t err_phiLMD = (-((MomRecLMD.Y() * fDP / cLm) / pow(fP, 2)) + (errMomRecLMD.Y() / cLm) / fP + (MomRecLMD.Y() * err_lyambdaLMD * TMath::Tan(fLm) / cLm) / fP) /
