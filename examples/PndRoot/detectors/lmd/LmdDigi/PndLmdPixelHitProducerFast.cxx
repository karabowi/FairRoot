...
// -----   Default constructor   -------------------------------------------
PndLmdPixelHitProducerFast::PndLmdPixelHitProducerFast() : PndSdsTask("Fast LMD Hit Producer")
{
...
PndLmdPixelHitProducerFast::PndLmdPixelHitProducerFast(Double_t pixelx, Double_t pixely) : PndSdsTask("Fast LMD Hit Producer"), fPixelSizeX(pixelx), fPixelSizeY(pixely)
{
  SetPersistency(kTRUE);
}
// -------------------------------------------------------------------------
...
// -----   Named constructor   -------------------------------------------
PndLmdPixelHitProducerFast::PndLmdPixelHitProducerFast(const char *name) : PndSdsTask(name)
{
  fPixelSizeX = 0.005; // to round MC-Hit x-position
  fPixelSizeY = 0.005; // to round MC-Hit y-position
  SetPersistency(kTRUE);
...
InitStatus PndLmdPixelHitProducerFast::Init()
{
  ...
  LOG(info) << " PndLmdPixelHitProducerFast: Intialisation successfull";
  return kSUCCESS;
}
// -------------------------------------------------------------------------
...
void PndLmdPixelHitProducerFast::Exec(Option_t *)
{
  ...
  for (Int_t iPoint = 0; iPoint < nPoints; iPoint++) {
    ...
    TVector3 position(fPixelSizeX * TMath::Nint(point->GetX() / fPixelSizeX), fPixelSizeY * TMath::Nint(point->GetY() / fPixelSizeY), point->GetZ());
