...
// -----   Default constructor   -------------------------------------------
PndLmdHybridHitProducer::PndLmdHybridHitProducer() : PndSdsHybridHitProducer("LMD Hybrid Hit Producer")
{
  fPixelHits = 0;
  fEventNr = 0;
  ...
  if (fVerbose > 2)
    std::cout << "LMD Hybrid Digi Producer created, Parameters will be taken from RTDB" << std::endl;
}
// -------------------------------------------------------------------------
...
{
  if (fVerbose > 1)
    Info("SetBranchNames", "Set Lmd Pixel names.");
