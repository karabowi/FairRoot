...
// -----   Default constructor   -------------------------------------------
PndLmdPixelClusterTask::PndLmdPixelClusterTask() : PndSdsPixelClusterTask("LMD Clustertisation Task")
{
...
void PndLmdPixelClusterTask::Exec(Option_t *)
{
  ...
  if (fVerbose > 0)
    LOG(info) << " PndSdsPixelClusterTask::Exec EventTime: " << EventTime;
  ...
  if (fVerbose > 1) {
    LOG(info) << " PndLmdPixelClusterTask: " << fClusterArray->GetEntriesFast() << " Sds Clusters and " << fHitArray->GetEntriesFast() << " Hits calculated.";
  }
  fEventNr++;
  fHitArray->Sort();
