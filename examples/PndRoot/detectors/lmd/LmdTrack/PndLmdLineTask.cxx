...
PndLmdLineTask::PndLmdLineTask(TString hitBranch) : FairTask("Connect 2 points task")
{
...
InitStatus PndLmdLineTask::Init()
{
  ...
  LOG(info) << " PndLmdLineTask: Initialisation successfull";
  return kSUCCESS;
}
...
void PndLmdLineTask::Exec(Option_t *)
{
  ...
  if (fTrackArray == nullptr)
    Fatal("PndLmdLineTask::Exec", "No TrackArray");
  fTrackArray->Delete();
  ...
  // Detailed output
  if (fVerbose > 1)
    std::cout << " -I- PndLmdLineTask: contains " << ntcand << " RhoCandidates" << std::endl;
  if (fVerbose > 2) {
  ...
  for (Int_t track = 0; track < ntcand; track++) {
    ...
    PndLinTrack *trackfit = new PndLinTrack("Lumi", posSeed.X(), dirSeed.X(), posSeed.Y(), dirSeed.Y(), posSeed.Z(), posSeed.Z(), 0, firstHit, lastHit, track);
