...
PndLmdLinFitTask::PndLmdLinFitTask() : FairTask("3D-Straight-Line-Fit"), fTCandArray(nullptr), fRecoArray(nullptr), fTrackArray(nullptr), fGeoH(nullptr)
{
...
PndLmdLinFitTask::PndLmdLinFitTask(TString tTCandBranchName, TString tRecoBranchName, TString tOutputBranchName, TString tOutputFolder)
  : FairTask("3D-Straight-Line-Fit"), fTCandArray(nullptr), fRecoArray(nullptr), fTrackArray(nullptr), fGeoH(nullptr)
{
...
InitStatus PndLmdLinFitTask::Init()
{
  ...
  LOG(info) << " PndLmdLinFitTask: Initialisation successfull";
  return kSUCCESS;
}
...
void PndLmdLinFitTask::Exec(Option_t *)
{
  ...
  if (fTrackArray == nullptr)
    Fatal("PndLmdLinFitTask::Exec", "No TrackArray");
  fTrackArray->Delete();
  ...
  // Detailed output
  if (fVerbose > 1)
    std::cout << " -I- PndLmdLinFitTask: contains " << ntcand << " RhoCandidates" << std::endl;
  if (fVerbose > 2) {
...
double PndLmdLinFitTask::distance_MS(double x, double y, double z, double errx, double erry, const double *p, double *zpr)
{
  ...
  for (int iz = 0; iz < 8; iz++)
    if (((p[4] + t_min) - zpr[iz]) <= 0)
      THfunc[iz] = 0;

  double funcX = p[0] + p[1] * t_min + (-p[6] + p[7]) * ((p[4] + t_min) - zpr[0]) * THfunc[0] + (-p[8] + p[9]) * ((p[4] + t_min) - zpr[2]) * THfunc[2] +
                 (-p[10] + p[11]) * ((p[4] + t_min) - zpr[4]) * THfunc[4] + (-p[12] + p[13]) * ((p[4] + t_min) - zpr[6]) * THfunc[6];
  double funcY = p[2] + p[3] * t_min + (-p[14] + p[15]) * ((p[4] + t_min) - zpr[0]) * THfunc[0] + (-p[16] + p[17]) * ((p[4] + t_min) - zpr[2]) * THfunc[2] +
                 (-p[18] + p[19]) * ((p[4] + t_min) - zpr[4]) * THfunc[4] + (-p[20] + p[21]) * ((p[4] + t_min) - zpr[6]) * THfunc[6];
...
double PndLmdLinFitTask::line3DfitMS(TGraph2DErrors *gr, const TVector3 &posSeed, const TVector3 &dirSeed, std::vector<double> &fitpar, TMatrixDSym *covmatrix)
{
  ...
  double errdz2 = pow(dp5_dp1, 2) * (*covmatrix)(1, 1) + pow(dp5_dp3, 2) * (*covmatrix)(3, 3) + 2 * fabs(dp5_dp1 * dp5_dp3 * (*covmatrix)(1, 3));
  ...
  if (fVerbose > 2) {
    ...
    cout << "[AFTER FIT] dir: (" << fitpar[1] << ", " << fitpar[3] << ", " << fitpar[5] << ")"
         << " sqrt(p1^2+p3^2+p5^2) = " << sqrt(fitpar[1] * fitpar[1] + fitpar[3] * fitpar[3] + fitpar[5] * fitpar[5]) << endl;
  }
  ///-------------------------------------------------------------
