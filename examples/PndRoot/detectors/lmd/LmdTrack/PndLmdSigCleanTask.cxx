...
void PndLmdSigCleanTask::Exec(Option_t *)
{
  ...
  for (Int_t iN = 0; iN < nGeaneTrks; iN++) { // loop over all reconstructed trks
    ...
    if (isClean == kTRUE && isCleanCand == kTRUE) {
      ...
      counterSigTrk++;
      if (fVerbose > 2)
        cout << "***** isClean TRUE *****" << endl;
    } else {
      new ((*fTrackParFinal)[counterSigTrk]) FairTrackParH(); // save nullptr
      counterSigTrk++;
      if (fVerbose > 2)
        cout << "***** isClean FALSE *****" << endl;
    }
  } // [END] loop over all reconstructed trks
  if (fVerbose > 2)
    cout << "PndLmdSigCleanTask::Exec END!" << endl;
}

void PndLmdSigCleanTask::Finish() {}

bool PndLmdSigCleanTask::Check(FairTrackParH *fTrk)
{ // answer TRUE means "it's a signal"
  bool res;
  if (fabs(fPbeam - 1.5) < 1e-1)
...
bool PndLmdSigCleanTask::CheckXY(FairTrackParP *fFittedTrkP)
{
  ...
  if (fVerbose > 0)
    cout << "fFittedTrkP.GetX() = " << fFittedTrkP->GetX() << " Xref = " << Xref << " diffX = " << diffX << endl;
  if (diffX > 3.0)
    dirOKx = false;
  if (fVerbose > 0)
    cout << "!YPhFilt!" << endl;
  ...
  if (fVerbose > 0)
    cout << "fFittedTrkP.GetY() = " << fFittedTrkP->GetY() << " Yref = " << Yref << " diffY = " << diffY << endl;
  if (diffY > 4.0)
    dirOKy = false;
