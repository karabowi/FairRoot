...
// -----   Default constructor   -------------------------------------------
PndLmdTrackFinderCATask::PndLmdTrackFinderCATask() : FairTask("LMD Track Finding Task (Cellular Automation)")
{
  missPlAlgo = false;
...
// -----   Constructor   -------------------------------------------
PndLmdTrackFinderCATask::PndLmdTrackFinderCATask(const bool missPl, const double setdmax, int innSensPP, int innP, TString hitBranch)
  : FairTask("LMD Track Finding Task (Cellular Automation) with/without <<missing "
             "planes>> algoritm")
{
  missPlAlgo = missPl;
...
bool PndLmdTrackFinderCATask::SortHitsByDetSimple2(std::vector<std::vector<Int_t>> &hitsd, Int_t nStripHits)
{
  ...
  for (Int_t iHit = 0; iHit < nStripHits; iHit++) {
    ...
    const PndLmdHitLocationInfo &hit_info = lmd_helper.getHitLocationInfo(sensid);
...
InitStatus PndLmdTrackFinderCATask::Init()
{
  ...
  LOG(info) << " PndLmdTrackFinderCATask: Initialisation successfull";
  if (missPlAlgo)
...
void PndLmdTrackFinderCATask::Evolution(int &pv0, int &pv1, int &pv0_n, int &pv1_n, bool)
{ // isstop // [R.K.03/2017] unused variable(s)
...
TClonesArray *PndLmdTrackFinderCATask::ForwardEvolution(TClonesArray *tCellArray, int niter)
{
  ...
  for (int itt = 0; itt < niter; itt++) { // should be infinite loop! due to small number of layers 1000
...
void PndLmdTrackFinderCATask::Exec(Option_t *)
{
  ...
  if (fVerbose > 4)
    cout << "track can contain " << pcmax << "+1 cells" << endl;
  ...
  for (unsigned int newpcmax = pcmax; newpcmax > 0; newpcmax--) { // loop over possible number of cells in trk
    ...
    if (fVerbose > 4)
      cout << "Now we are looking for trk with max " << cur_max_tag + 1 << " cells among " << nCells << " cells" << endl;
    //    bool newtrk=true;
  ...
  if (fVerbose > 4)
    cout << "--- filter trk-cand array: " << endl;
  if (fVerbose > 4)
  ...
  for (unsigned int itrk = 0; itrk < trk_cells.at(0).size(); itrk++) {
    ...
    while (curr_arr > 0) {
      curr_arr--;
      if (fVerbose > 4)
        cout << " we have: trk_cells.at(" << curr_arr << ").at(" << itrk << ")=" << trk_cells.at(curr_arr).at(itrk) << endl;
      if (trk_cells.at(curr_arr).at(itrk) < 0)
        maxcellnum--;
    }
    ...
    if (fVerbose > 4)
      cout << " with:" << maxcellnum << " cells" << endl;
  }
  ...
  for (int itrc = (cell_parts.size() - 1); itrc >= 0; itrc--) {
    ...
    for (int itrc2 = cup; itrc2 >= 0; itrc2--) {
      ...
      if (cell_parts[itrc] < cell_parts[itrc2]) {
        ...
        if (fVerbose > 3)
          cout << "Cand.No" << itrc << ": Number of repeating cells = " << count_re << " and cells in tot = " << cell_parts[itrc] << " Number of repeating hits = " << count_hits
               << endl;
        if (count_re > 0.55 * cell_parts[itrc] || count_hits > 2) {
          trk_accept[itrc] = false;
          if (fVerbose > 4) {
            cout << "Delete: trk-cand#" << itrc << " because it contains (" << count_re << ") more then 55% cells from trk-cand#" << itrc2 << endl;
          }
        }
      }
    }
  ...
  for (unsigned int itrk = 0; itrk < trk_accept.size(); itrk++) {
    ...
    for (unsigned int icell = 0; icell < trk_arr_size; icell++) {
      ...
      if (firstHit) { // fill trk-cand seed
        ...
        if (fVerbose > 2)
          cout << " " << cell->GetHitDw();
        myTCand->AddHit(myHit->GetDetectorID(), cell->GetHitDw(), myHit->GetPosition().Z());
      ...
      if (fVerbose > 2)
        cout << " " << cell->GetHitUp();
      PndSdsHit *myHit1 = (PndSdsHit *)fStripHitArray->At(cell->GetHitUp());
      myTCand->AddHit(myHit1->GetDetectorID(), cell->GetHitUp(), myHit1->GetPosition().Z());
    }
    if (fVerbose > 2)
      cout << " " << endl;
