...
// -----   Default constructor   -------------------------------------------
PndLmdTrackFinderTask::PndLmdTrackFinderTask(Int_t inFinderMode, TString hitBranch, Int_t innSensPP) : FairTask("LMD Track Finding Task"), nSensPP(innSensPP)
{
...
InitStatus PndLmdTrackFinderTask::Init()
{
  ...
  LOG(info) << " PndLmdTrackFinderTask: Initialisation successfull";
  return kSUCCESS;
}
// -------------------------------------------------------------------------
...
// --------------------------------------------
void PndLmdTrackFinderTask::FindHitsIII(std::vector<PndTrackCand> &tofill, std::vector<std::vector<std::pair<Int_t, bool>>> &hitsd,
                                        Int_t) // nStripHits //FIXME [R.K.03/2017] unused variable(s)
{
  ...
  if (fVerbose > 1)
    cout << "Pseudos L2L3: " << trackStart.size() << endl;
  ...
  for (size_t i = 0; i < trackStart.size(); i++) // pseudo-loop
  {
    ...
    for (Int_t idet = 3; idet < 4; idet++) { // i know this is not a loop, but in case we someday will
                                             // have more planes
    ...
    if (ids.size() > 2) { // third hit found <=> !track found!
      ...
      for (size_t id = 0; id < ids.size(); id++) {
        ...
        myTCand->AddHit(myHit->GetDetectorID(), ids.at(id), myHit->GetPosition().Mag());
      }
...
// --------------------------------------------
void PndLmdTrackFinderTask::FindHitsII(std::vector<PndTrackCand> &tofill, std::vector<std::vector<std::pair<Int_t, bool>>> &hitsd,
                                       Int_t) // nStripHits //FIXME [R.K.03/2017] unused variable(s)
{
  std::vector<TVector3> trackStart, trackVec;   // pseudo tracks
  std::vector<TVector3> trackStartd, trackVecd; // save errors
...
void PndLmdTrackFinderTask::FindHitsI(std::vector<PndTrackCand> &tofill, std::vector<std::vector<std::pair<Int_t, bool>>> &hitsd,
                                      Int_t) // nStripHits //FIXME [R.K.03/2017] unused variable(s)
{
  std::vector<TVector3> trackStart, trackVec;   // pseudo tracks
  std::vector<TVector3> trackStartd, trackVecd; // save errors
