...
PndLmdKalmanTask::PndLmdKalmanTask() : FairTask("Kalman Filter"), fPersistence(kFALSE)
{
...
PndLmdKalmanTask::PndLmdKalmanTask(TString HitBranch, TString TrackBranch) : FairTask("Kalman Filter"), fPersistence(kFALSE)
{
...
{
  // if(fPH!=nullptr)delete fPH;
  // if(fChi2H!=nullptr)delete fChi2H;
  //  delete rep;
}
...
InitStatus PndLmdKalmanTask::Init()
{
  ...
  if (stripar == nullptr) { // TODO Convention on detector number needed
...
void PndLmdKalmanTask::Exec(Option_t *)
{
  ...
  // Detailed output
  if (fVerbose > 1)
    std::cout << " -I- PndLmdKalmanTask: contains " << ntracks << " Tracks." << std::endl;
  GFKalman fitter;
  int numIttr = 1;
  ...
  for (Int_t itr = 0; itr < ntracks; ++itr) {
    ...
    TVector3 StartDirErr(0.1 * (sqrt(hitCov[0][0])), 0.1 * (sqrt(hitCov[1][1])),
                         0.1 * sqrt(hitCov[2][2])); // TODO: check this assumption (2*sigma_{x}/20cm)
    ...
    if (flGEANE) {
      ...
      rep = new GeaneTrackRep(fPro, start_pl, StartMom, StartPosErr, StartMomErr, fCharge, fPDGCode);
    }
    /// GEANE track rep (END) --------------------
