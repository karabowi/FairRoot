...
Bool_t PndLmdAlignPar::getParams(FairParamList *list)
{
  if (!list)
    return kFALSE;
  if (!list->fill("num_halfs", &fhalfs))
    return kFALSE;
  if (!list->fill("num_planes", &fplanes))
    return kFALSE;
  if (!list->fill("num_modules", &fmodules))
    return kFALSE;
  if (!list->fill("num_sensors", &fsensors))
    return kFALSE;
  if (!list->fill("num_sides", &fsides))
    return kFALSE;
  ...
  for (int ihalf = 0; ihalf < fhalfs; ihalf++) {             // loop over detector halves
    for (int iplane = 0; iplane < fplanes; iplane++) {       // loop over planes
      for (int imodule = 0; imodule < fmodules; imodule++) { // loop over modules
        ...
        if (!list->fill(name_shx, &fshift_x[curpos]))
          return kFALSE;
        if (!list->fill(name_shy, &fshift_y[curpos]))
          return kFALSE;
        if (!list->fill(name_shz, &fshift_z[curpos]))
          return kFALSE;
        if (!list->fill(name_rotx, &frotate_x[curpos]))
          return kFALSE;
        if (!list->fill(name_roty, &frotate_y[curpos]))
          return kFALSE;
        if (!list->fill(name_rotz, &frotate_z[curpos]))
          return kFALSE;
      }
    }
  }
...
void PndLmdAlignPar::Print()
{
  ...
  for (size_t ix = 0; ix < fshift_x.size(); ix++) {
    if (fshift_x[ix] != 0)
      cout << "fshift_x[" << ix << "]=" << fshift_x[ix] << endl;
    if (fshift_y[ix] != 0)
      cout << "fshift_y[" << ix << "]=" << fshift_y[ix] << endl;
    if (fshift_z[ix] != 0)
      cout << "fshift_z[" << ix << "]=" << fshift_z[ix] << endl;
    if (frotate_x[ix] != 0)
      cout << "frotate_x[" << ix << "]=" << frotate_x[ix] << endl;
    if (frotate_y[ix] != 0)
      cout << "frotate_y[" << ix << "]=" << frotate_y[ix] << endl;
    if (frotate_z[ix] != 0)
      cout << "frotate_z[" << ix << "]=" << frotate_z[ix] << endl;
  }
  return;
}
