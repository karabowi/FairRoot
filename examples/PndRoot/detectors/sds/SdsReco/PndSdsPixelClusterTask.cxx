...
PndSdsPixelClusterTask::PndSdsPixelClusterTask()
  : PndSdsTask("SDS Clustertisation Task"), fUseNoiseSuppression(kTRUE), fFEModel(nullptr), fDigiArray(nullptr), fDigiPar(nullptr), fChargeConverter(nullptr), fGeoH(nullptr),
    fClustBranchName(""), fClusterArray(nullptr), fHitArray(nullptr), fEventHeader(0), fFunctor(nullptr), fStartFunctor(nullptr), fClusterType(-1), fEventNr(0),
    fClusterFinder(nullptr), fBackMapping(nullptr)
{
...
PndSdsPixelClusterTask::PndSdsPixelClusterTask(const char *name)
  : PndSdsTask(name), fUseNoiseSuppression(kTRUE), fFEModel(nullptr), fDigiArray(nullptr), fDigiPar(nullptr), fChargeConverter(nullptr), fGeoH(nullptr), fClustBranchName(""),
    fClusterArray(nullptr), fHitArray(nullptr), fEventHeader(0), fFunctor(nullptr), fStartFunctor(nullptr), fClusterType(-1), fEventNr(0), fClusterFinder(nullptr),
    fBackMapping(nullptr)
{
  SetPersistency(kTRUE);
}
// -------------------------------------------------------------------------
...
PndSdsPixelClusterTask::~PndSdsPixelClusterTask()
{
  if (fBackMapping != nullptr)
    delete fBackMapping;
  if (fClusterFinder != nullptr)
...
void PndSdsPixelClusterTask::GetParList(TList *)
{ // tempList //[R.K.03/2017] unused variable(s)
}
...
void PndSdsPixelClusterTask::SetParContainers()
{
  ...
  if (!fDigiPar)
    Fatal("SetParContainers", "No digitization parameters specified");
  if (fVerbose > 1)
...
InitStatus PndSdsPixelClusterTask::Init()
{
  ...
  LOG(info) << " PndSdsPixelClusterTask: Initialisation successfull";
  return kSUCCESS;
}
// -------------------------------------------------------------------------
...
void PndSdsPixelClusterTask::Exec(Option_t *)
{
  ...
  if (fVerbose > 0)
    LOG(info) << " PndSdsPixelClusterTask::Exec EventNumber: " << entryNumber;
    ...
    for (UInt_t i = 0; i < clusterSize; i++) {
      if (clusters[i].size() == 1) {
        ...
        if (tempDigi.GetCharge() < 40) {
          if (fVerbose > 2)
            LOG(info) << "  PndSdsPixelClusterTask::Exec():Erased cluster with digi " << tempDigi;
  ...
  if (fVerbose > 1) {
    LOG(info) << " PndSdsPixelClusterTask: " << fClusterArray->GetEntriesFast() << " Sds Clusters and " << fHitArray->GetEntriesFast() << " Hits calculated.";
  }
  fEventNr++;
  fHitArray->Sort();
  return;
