...
// -----   Default constructor   -------------------------------------------
PndSdsRecoTask::PndSdsRecoTask() : fHitCovMatrix(3, 3), PndSdsTask("Reconstruction task for PANDA PndSds")
{
  /*  fSigmaX=0.;
    fSigmaY=0.;
    fSigmaZ=0.;*/
...
*/
// -----   Constructor   ---------------------------------------------------
PndSdsRecoTask::PndSdsRecoTask(std::string type) : PndSdsTask("Reconstruction task for PANDA PndSds")
{
  fMakerType = type;
  if (fVerbose > 0)
...
InitStatus PndSdsRecoTask::Init()
{
  ...
  if (fDigiPixelArray == nullptr) {
    LOG(warn) << " PndSdsRecoTask::Init: No fDigiPixelArray!";
    return kERROR;
  }

  // Get input array
  fDigiStripArray = (TClonesArray *)ioman->GetObject(fStripBranchName);
  if (fDigiStripArray == nullptr) {
    LOG(warn) << " PndSdsRecoTask::Init: No fDigiStripArray!";
    return kERROR;
  }
...
void PndSdsRecoTask::Exec(Option_t *)
{
  ...
  if (!fDigiPixelArray)
    Fatal("Exec", "No fDigiPixelArray");
  if (!fDigiStripArray)
    Fatal("Exec", "No fDigiStripArray");
...
void PndSdsRecoTask::CalcDetPlane(TVector3 &oVect, TVector3 &uVect, TVector3 &vVect)
{
  ...
  if (fVerbose > 1) {
    std::cout << "PndSdsRecoTask::CalcDetPlane from Detector " << fCurrentPndSdsMCPoint->GetDetName() << std::endl;
  }
  // make transformation
