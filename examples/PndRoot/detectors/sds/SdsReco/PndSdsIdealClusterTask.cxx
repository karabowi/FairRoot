...
PndSdsIdealClusterTask::PndSdsIdealClusterTask()
  : PndSdsTask("SDS Clustertisation Task"), fDigiArray(nullptr), fClustBranchName(""), fClusterType(-1), fClusterArray(nullptr), fHitArray(nullptr),
    fGeoH(PndGeoHandling::Instance()), fRadius(0), fFEcolumns(0), fFErows(0), fGeoFile(""), finder(nullptr), mapping(nullptr)
{
...
PndSdsIdealClusterTask::PndSdsIdealClusterTask(Double_t radius, Int_t FEcolumns, Int_t FErows, TString geoFile)
  : PndSdsTask("SDS Clustertisation Task"), fDigiArray(nullptr), fClustBranchName(""), fClusterType(-1), fClusterArray(nullptr), fHitArray(nullptr),
    fGeoH(PndGeoHandling::Instance()), fRadius(radius), fFEcolumns(FEcolumns), fFErows(FErows), fGeoFile(geoFile), finder(nullptr), mapping(nullptr)
{
  // TGeoManager* geoMan;// = new TGeoManager("geoMan","geoMan");
  // geoMan = gGeoManager;
  // fGeoH = PndGeoHandling::Instance();
...
InitStatus PndSdsIdealClusterTask::Init()
{
  ...
  LOG(info) << " PndSdsIdealClusterTask: Initialisation successfull";
  return kSUCCESS;
}
// -------------------------------------------------------------------------
