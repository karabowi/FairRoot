...
PndSdsIdealRecoTask::PndSdsIdealRecoTask()
  : PndSdsTask("Ideal reconstruction task for PANDA PndSds"), fGeoH(PndGeoHandling::Instance()), fPointArray(nullptr), fMctruthArray(nullptr), fHitOutputArray(nullptr),
    fSigmaX(0.), fSigmaY(0.), fSigmaZ(0.), fCurrentPndSdsMCPoint(nullptr), fCurrentTransMat(nullptr), fHitCovMatrix(3, 3)
{
...
PndSdsIdealRecoTask::PndSdsIdealRecoTask(Double_t sx, Double_t sy, Double_t sz)
  : PndSdsTask("Ideal reconstruction task for PANDA PndSds"), fGeoH(PndGeoHandling::Instance()), fPointArray(nullptr), fMctruthArray(nullptr), fHitOutputArray(nullptr),
    fSigmaX(sx), fSigmaY(sy), fSigmaZ(sz), fCurrentPndSdsMCPoint(nullptr), fCurrentTransMat(nullptr), fHitCovMatrix(3, 3)
{
  // fGeoH = PndGeoHandling::Instance();
  SetPersistency(kTRUE);
}
...
InitStatus PndSdsIdealRecoTask::Init()
{
  ...
  if (!fPointArray) {
    std::cout << "-W- PndSdsIdealRecoTask::Init: "
  ...
  if (fMctruthArray == nullptr) {
    LOG(warn) << " PndSdsIdealRecoTask::Init: No McTruth array!";
    return kERROR;
  }
...
void PndSdsIdealRecoTask::Exec(Option_t *)
{
  ...
  for (Int_t iMvdPoint = 0; iMvdPoint < nPndSdsHits; ++iMvdPoint) {
    ...
    new ((*fHitOutputArray)[size])
      PndSdsHit(fCurrentPndSdsMCPoint->GetDetectorID(), fCurrentPndSdsMCPoint->GetSensorID(), pos, dposLocal, -1, fCurrentPndSdsMCPoint->GetEnergyLoss(), 1, iMvdPoint);
...
void PndSdsIdealRecoTask::InitTransMat()
{
  ...
  gGeoManager->cd(fGeoH->GetPath(fCurrentPndSdsMCPoint->GetSensorID()).Data());
...
void PndSdsIdealRecoTask::smear(TVector3 &pos)
{
  ...
  if (fVerbose > 1) {
    std::cout << "PndSdsIdealRecoTask::smear Point (x,y,z)=(" << pos.x() << "," << pos.z() << "," << pos.z() << ") by (" << fSigmaX << "," << fSigmaY << "," << fSigmaZ << ") to ";
  }
  pos.SetXYZ(x, y, z);
  if (fVerbose > 1) {
...
void PndSdsIdealRecoTask::CalcDetPlane(TVector3 &oVect, TVector3 &uVect, TVector3 &vVect)
{
  ...
  if (fVerbose > 1) {
    std::cout << "PndSdsIdealRecoTask::CalcDetPlane from Detector " << fCurrentPndSdsMCPoint->GetSensorID() << std::endl;
  }
  // make transformation
