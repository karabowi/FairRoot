...
PndSdsStripDigiPar::PndSdsStripDigiPar(const char *name, const char *title, const char *context)
  : FairParGenericSet(name, title, context), fTopPitch(0.), fBotPitch(0.), fOrient(0.), fSkew(0.), fThreshold(0.), fNoise(0.), fCSigma(0.), fChargeCut(0.), fSingleChargeCut(0.),
    fFeBusClock(0.), fTopAnchor(0., 0.), fBotAnchor(0., 0.), fFeChannels(0), fTopNrFE(0), fBotNrFE(0), fClusterMod(0), fClusterMean(0), fRadChannel(0), fRadTime(0), fSensType(""),
    fFeType(""), fChargeConvMethod(0)
{
  clear();
}
...
Bool_t PndSdsStripDigiPar::getParams(FairParamList *list)
{
  ...
  if (!list->fill("top_pitch", &fTopPitch))
    return kFALSE;
  if (!list->fill("bot_pitch", &fBotPitch))
    return kFALSE;
  if (!list->fill("orient", &fOrient))
    return kFALSE;
  if (!list->fill("skew", &fSkew))
    return kFALSE;
  Double_t x, y;
  ...
  if (!list->fill("bot_anchor_y", &y))
    return kFALSE;
  fBotAnchor.Set(x, y);
  if (!list->fill("nr_fe_channels", &fFeChannels))
    return kFALSE;
  if (!list->fill("nr_fe_top", &fTopNrFE))
    return kFALSE;
  if (!list->fill("nr_fe_bottom", &fBotNrFE))
    return kFALSE;
  ...
  if (!list->fill("fe_BusClock", &fFeBusClock))
    return kFALSE;
  if (!list->fill("cluster_mod", &fClusterMod))
    return kFALSE;
  if (!list->fill("cluster_mean", &fClusterMean))
    return kFALSE;
  if (!list->fill("cluster_radchan", &fRadChannel))
    return kFALSE;
  if (!list->fill("cluster_radtime", &fRadTime))
    return kFALSE;
  if (!list->fill("cluster_corrchargecut", &fChargeCut))
    return kFALSE;
  if (!list->fill("cluster_singlechargecut", &fSingleChargeCut))
    return kFALSE;
