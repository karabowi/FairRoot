...
PndSdsDetector::PndSdsDetector()
  : FairDetector(), fPersistance(kTRUE), fTrackID(-1), fVolumeID(-1), fPosIn(), fPosOut(), fMomIn(), fMomOut(), fTime(0.), fLength(0.), fELoss(0.),
    fGeoH(PndGeoHandling::Instance()), fPosIndex(0), fUseRadDamOption(false), fOutBranchName(""), fFolderName(""), fDetectorID(), fListOfSensitives()
{
}
// -------------------------------------------------------------------------
...
PndSdsDetector::PndSdsDetector(const char *name, Bool_t active)
  : FairDetector(name, active), fPersistance(kTRUE), fTrackID(-1), fVolumeID(-1), fPosIn(), fPosOut(), fMomIn(), fMomOut(), fTime(0.), fLength(0.), fELoss(0.),
    fGeoH(PndGeoHandling::Instance()), fPosIndex(0), fUseRadDamOption(false), fOutBranchName(""), fFolderName(""), fDetectorID(), fListOfSensitives()
{
}
// -------------------------------------------------------------------------

PndSdsDetector::PndSdsDetector(const PndSdsDetector& rhs)
    : FairDetector(rhs)
    , fPosIndex(0)
...
Bool_t PndSdsDetector::ProcessHits(FairVolume *vol)
{
  ...
  if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
    ...
    AddHit(fTrackID, FairRootManager::Instance()->GetBranchId("MCTrack"), fGeoH->GetShortID(detPath), TVector3(fPosIn.X(), fPosIn.Y(), fPosIn.Z()),
...
{
  FairRootManager::Instance()->RegisterAny(fOutBranchName, fPointVector, kTRUE);
}
// -------------------------------------------------------------------------
...
void PndSdsDetector::CopyClones(TClonesArray *cl1, TClonesArray *cl2, Int_t offset)
{
  ...
  LOG(info) << " PndSdsDetector: " << nEntries << " entries to add.";

  TClonesArray &clref = *cl2;

  PndSdsMCPoint *oldpoint = nullptr;
  ...
  LOG(info) << " PndSdsDetector: " << cl2->GetEntriesFast() << " merged entries.";
}
...
void PndSdsDetector::SetExclusiveSensorType(const TString sens)
{
  ...
  LOG(info) << " PndSdsDetector: Only active sensor type is set to \"" << sens.Data() << "\",";
...
void
PndSdsDetector::AddHit(Int_t trackID, Int_t detID, Int_t sensorID, TVector3 posIn, TVector3 posOut, TVector3 momIn, TVector3 momOut, Double_t time, Double_t length, Double_t eLoss)
{
    fPointVector->push_back(PndSdsMCPoint(trackID, detID, sensorID, posIn, posOut, momIn, momOut, time, length, eLoss));
}
// -------------------------------------------------------------------------

Bool_t PndSdsDetector::IsSensitive(const std::string& name)
{
  for (UInt_t i = 0; i < fListOfSensitives.size(); i++) {
    if (name.find(fListOfSensitives[i]) != std::string::npos)
      return true;
