...
PndSdsHybridHitProducer::PndSdsHybridHitProducer()
  : PndSdsTask("SDS Hybrid Hit Producer"), fDigiPixelMCInfo(kFALSE), fPointArray(nullptr), fPixelArray(nullptr), fPixelMCArray(nullptr), fDigiPar(nullptr), fTotDigiPar(nullptr),
    fChargeConverter(nullptr), fMCEventHeader(nullptr), fDataBuffer(nullptr), flx(0.), fly(0.), fthreshold(0.), fnoise(0.), fqsigma(0.), fcols(0), frows(0), fPixelHits(0),
    fGeoH(nullptr), fEventNr(0), fOverwriteParams(kFALSE), fTimeOrderedDigi(kFALSE), fPixelList()
{
  if (fVerbose > 0)
    Info("PndSdsHybridHitProducer", "SDS Hybrid Digi Producer created, Parameters will be taken from RTDB");
  SetPersistency(kTRUE);
}
// -------------------------------------------------------------------------

PndSdsHybridHitProducer::PndSdsHybridHitProducer(const char *name)
  : PndSdsTask(name), fDigiPixelMCInfo(kFALSE), fPointArray(nullptr), fPixelArray(nullptr), fPixelMCArray(nullptr), fDigiPar(nullptr), fTotDigiPar(nullptr),
    fChargeConverter(nullptr), fMCEventHeader(nullptr), fDataBuffer(nullptr), flx(0.), fly(0.), fthreshold(0.), fnoise(0.), fqsigma(0.), fcols(0), frows(0), fPixelHits(0),
    fGeoH(nullptr), fEventNr(0), fOverwriteParams(kFALSE), fTimeOrderedDigi(kFALSE), fPixelList()
{
  if (fVerbose > 0)
    Info("PndSdsHybridHitProducer", "%s created, Parameters will be taken from RTDB", name);
  SetPersistency(kTRUE);
}
// -------------------------------------------------------------------------

PndSdsHybridHitProducer::PndSdsHybridHitProducer(Double_t lx, Double_t ly, Double_t threshold, Double_t noise)
  : PndSdsTask("SDS Hybrid Digi Producer (PndSdsHybridHitProducer)"), fDigiPixelMCInfo(kFALSE), fPointArray(nullptr), fPixelArray(nullptr), fPixelMCArray(nullptr),
    fDigiPar(nullptr), fTotDigiPar(nullptr), fChargeConverter(nullptr), fMCEventHeader(nullptr), fDataBuffer(nullptr), flx(lx), fly(ly), fthreshold(threshold), fnoise(noise),
    fqsigma(0.), fcols(104), frows(104), fPixelHits(0), fGeoH(nullptr), fEventNr(0), fOverwriteParams(kTRUE), fTimeOrderedDigi(kFALSE), fPixelList()
{
  fTimeOrderedDigi = kFALSE;
  if (fVerbose > 0)
    Info("PndSdsHybridHitProducer", "SDS Hybrid Digi Producer created, Parameters will be overwritten in RTDB");
  SetPersistency(kTRUE);
}
// -------------------------------------------------------------------------
...
PndSdsHybridHitProducer::~PndSdsHybridHitProducer()
{
  if (fChargeConverter != nullptr)
    delete fChargeConverter;
...
void PndSdsHybridHitProducer::SetParContainers()
{
  ...
  if (!fDigiPar)
    Fatal("SetParContainers", "No digitiztion parameters specified.");
  if (fVerbose > 1)
...
void PndSdsHybridHitProducer::Exec(Option_t *)
{
  ...
  if (fVerbose > 0) {
    std::cout << std::endl;
    LOG(info) << " PndSdsHybridHitProducer::Exec EventTime: " << EventTime << " event: " << FairRootManager::Instance()->GetEntryNr();
  }
  ...
  if (!fChargeConverter)
    Fatal("Exec", "No charge converter specified");
  ...
  if (fVerbose > 3)
    Info("Exec", "Begin loop for %i points", nPoints);
  for (Int_t iPoint = 0; iPoint < nPoints; iPoint++) {
    ...
    if (fGeoH->GetPath(point->GetSensorID()).Contains("Pixel") ||
        (fGeoH->GetPath(point->GetSensorID()).Contains("Sensor") && !fGeoH->GetPath(point->GetSensorID()).Contains("Strip"))) {
      ...
      if (fqsigma > 0) {
        ...
        myPixels = PixelCalc.GetPixels(posInL.getX(), posInL.getY(), posOutL.getX(), posOutL.getY(), point->GetEnergyLoss());
      } else {
        // Define sensor by pixelsizes threshold and noise from macro outside
  ...
  for (unsigned int iPix = 0; iPix < fPixelList.size(); iPix++) {
    ...
    //			std::cout << "FEModel-Test: " << point->GetEnergyLoss() << " " << charge << " " << fChargeConverter->ChargeToDigiValue(charge) << " " <<
    // fFEModel->GetTotFromCharge(charge) << " " << fFEModel->GetChargeFromTot(fFEModel->GetTotFromCharge(charge)) << std::endl
    ...
    Double_t timeStamp = (Double_t)fChargeConverter->GetTimeStamp(point->GetTime(), smearedChargeInE, EventTime);
    ...
    if (fVerbose > 2) {
      std::cout << "TimeStampCalc: EventTime: " << EventTime << " ToF " << point->GetTime() << " charge " << smearedChargeInE << " TW: " << timewalk
    ...
    PndSdsDigiPixel *tempPixel =
      new PndSdsDigiPixel(fPixelList[iPix].GetMCIndex(), fInBranchId, fPixelList[iPix].GetSensorID(), fPixelList[iPix].GetFE(), fPixelList[iPix].GetCol(),
                          fPixelList[iPix].GetRow(), smearedCharge, correctedTimeStamp); // fChargeConverter->GetTimeStamp(point->GetTime(), charge,fEventHeader->GetEventTime()) );
  ...
  if (fVerbose > 1) {
    std::cout << "-I- PndSdsHybridHitProducer: " << nPoints << " PndSdsMCPoints, " << fPixelHits << " Digi created."
              << " " << iFePixel << "  (event " << fEventNr << ")" << std::endl;
  }
  fEventNr++;
  if (fVerbose > 3)
...
void PndSdsHybridHitProducer::AddHits(std::vector<PndSdsPixel> *hits, int mcIndex)
{
  ...
  if (fVerbose > 1)
    std::cout << "Size of fPixelList: " << fPixelList.size() << std::endl;
}
...
void PndSdsHybridHitProducer::AddHit(PndSdsPixel &hit, int mcIndex)
{
  bool found = false;
  if (fVerbose > 1)
    std::cout << "Adding Hits to PixelList with mcIndex: " << mcIndex << std::endl;
  // check if the channel fired already
  for (UInt_t i = 0; i < fPixelList.size(); i++) {
    if (fPixelList[i].GetSensorID() == hit.GetSensorID() && fPixelList[i].GetFE() == hit.GetFE() && fPixelList[i].GetCol() == hit.GetCol() &&
        fPixelList[i].GetRow() == hit.GetRow()) {
      if (fVerbose > 1)
        std::cout << "Pixel " << hit.GetSensorID() << " FE/col/row " << hit.GetFE() << "/" << hit.GetCol() << "/" << hit.GetRow() << " already hit!" << std::endl;
