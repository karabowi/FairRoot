...
PndSdsCalcPixel::PndSdsCalcPixel()
  : fPixels(), fPixelWidth(0.), fPixelLength(0.), fThreshold(0.), fNoise(0.), fEnergy(0.), fVerboseLevel(0), fQuad(kQuadUNDEF), fNextPixel(kPixelUNDEF), fIn(), fOut(), fDir(),
    fPos(), fCon(), fActivePixel(), fCperL(-1), fStop(false)
{
}

PndSdsCalcPixel::PndSdsCalcPixel(Double_t w, Double_t l, Double_t threshold, Double_t noise)
  : fPixels(), fPixelWidth(w), fPixelLength(l), fThreshold(threshold), fNoise(noise), fEnergy(0.), fVerboseLevel(0), fQuad(kQuadUNDEF), fNextPixel(kPixelUNDEF), fIn(), fOut(),
    fDir(), fPos(), fCon(), fActivePixel(), fCperL(-1), fStop(false)
{
}

Int_t PndSdsCalcPixel::GetPixelsAlternative(Double_t inx, Double_t iny, Double_t outx, Double_t outy, Double_t energy, std::vector<Int_t> &cols, std::vector<Int_t> &rows,
                                            std::vector<Double_t> &charges)
{
  ...
  for (Int_t i = 0; i < npix; i++) {
    if (fVerboseLevel > 2)
      Info("PndSdsCalcPixel::GetPixelsAlternative()", "pass this pixel: i=%i, c=%i, r=%i, q=%f", i, pixels[i].GetCol(), pixels[i].GetRow(), pixels[i].GetCharge());
...
std::vector<PndSdsPixel> PndSdsCalcPixel::GetPixels(Double_t inx, Double_t iny, Double_t outx, Double_t outy, Double_t energy)
{
  fPixels.clear();
...
void PndSdsCalcPixel::CalcPixel()
{
  ...
  switch (fQuad) {
  case kUR:
    borderX = fPixelLength * (fActivePixel.GetCol() + 1);
    borderY = fPixelWidth * (fActivePixel.GetRow() + 1);
    if (fabs(fDir.getX()) < 1e-8)
      xBeforeY = false;
    ...
    break;
  case kUL:
    borderX = fPixelLength * (fActivePixel.GetCol());
    borderY = fPixelWidth * (fActivePixel.GetRow() + 1);
    if (fabs(fDir.getX()) < 1e-8)
      xBeforeY = false;
    else if (((borderX - fPos.getX()) * fDir.getY() / fDir.getX()) + fPos.getY() < borderY)
  ...
  case kDR:
    ...
    break;
  case kQuadUNDEF:
    std::cout << "PndSdsCalcPixel::CalcPixel : Quadrant not defined!" << std::endl;
...
{
  out << "PixelWidth: " << fPixelWidth << " PixelLength: " << fPixelLength << std::endl;
