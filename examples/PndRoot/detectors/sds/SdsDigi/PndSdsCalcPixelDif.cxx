...
PndSdsCalcPixelDif::PndSdsCalcPixelDif()
  : fPixels(), fActivePixel(), fPixelSizeX(0.), fPixelSizeY(0.), fRows(0.), fCols(0.), fThreshold(0.), fNoise(0.), fQspread(0.), fEnergy(0.), fVerboseLevel(0)
{
}

PndSdsCalcPixelDif::PndSdsCalcPixelDif(const PndSdsPixelDigiPar &digi_par)
  : fPixels(), fActivePixel(), fPixelSizeX(digi_par.GetXPitch()), fPixelSizeY(digi_par.GetYPitch()), fRows(digi_par.GetFERows() * digi_par.GetMaxFEperRow()),
    fCols(digi_par.GetFECols() * digi_par.GetMaxFEperCol()), fThreshold(digi_par.GetThreshold()), fNoise(digi_par.GetNoise()), fQspread(digi_par.GetQCloudSigma()), fEnergy(0.),
    fVerboseLevel(0)
{
}
...
PndSdsCalcPixelDif::PndSdsCalcPixelDif(Double_t lx, Double_t ly, Double_t qspread, Double_t threshold, Double_t noise, Double_t nrows, Double_t ncols)
  : fPixels(), fActivePixel(), fPixelSizeX(lx), fPixelSizeY(ly), fRows(nrows), fCols(ncols), fThreshold(threshold), fNoise(noise), fQspread(qspread), fEnergy(0.), fVerboseLevel(0)
{
}

Int_t PndSdsCalcPixelDif::GetPixelsAlternative(Double_t inx, Double_t iny, Double_t outx, Double_t outy, Double_t energy, std::vector<Int_t> &cols, std::vector<Int_t> &rows,
                                               std::vector<Double_t> &charges)
{
  ...
  for (Int_t i = 0; i < npix; i++) {
    if (fVerboseLevel > 2)
      Info("PndSdsCalcPixelDif::GetPixelsAlternative()", "pass this pixel: i=%i, c=%i, r=%i, q=%f", i, pixels[i].GetCol(), pixels[i].GetRow(), pixels[i].GetCharge());
...
std::vector<PndSdsPixel> PndSdsCalcPixelDif::GetPixels(Double_t inx, Double_t iny, Double_t outx, Double_t outy, Double_t dE)
{
  fPixels.clear();
  ...
  unsigned int cols(((unsigned int)std::fabs(max_x / fPixelSizeX)) - ((unsigned int)std::fabs(min_x / fPixelSizeX)) + 1);
  unsigned int rows(((unsigned int)std::fabs(max_y / fPixelSizeY)) - ((unsigned int)std::fabs(min_y / fPixelSizeY)) + 1);
  ...
  if (hitstorage.size() == 1) {
    ...
    for (unsigned int i = 0; i < samples; ++i) {
      ...
      if (std::abs(difx) > max_charge_diffusion_distance || std::abs(dify) > max_charge_diffusion_distance)
        continue;
...
void PndSdsCalcPixelDif::InjectPixelCharge(Int_t col, Int_t row, Double_t charge)
{
  ...
  if (col < 0 || row < 0 || col > fCols || row > fRows) {
    if (fVerboseLevel > 2) {
      std::stringstream ss;
      ss << " (col=" << col << ",row=" << row << " values exceed sensor boundaries: cols=[0-" << fCols << "] rows=[0-" << fRows << "]";
  ...
  if (fVerboseLevel > 3)
    std::cout << " charge = " << charge << ", smeared = " << smearedCharge << std::endl;
  if (smearedCharge > fThreshold) {
    if (fVerboseLevel > 3)
      Info("PndSdsCalcPixelDif::InjectPixelCharge", "col=%i, row=%i,charge=%f", col, row, charge);
