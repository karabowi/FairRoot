...
/// Default constructor
PndSdsTotChargeConversion::PndSdsTotChargeConversion(Int_t VerboseLevel)
  : PndSdsChargeConversion(kToT), fthreshold(0.), Qt(0.), t1e(0.), t2e(0.), ftimestep(0.), fstarttime(0.), fstoptime(0.), ftimeoffset(0.), ftimewalk(0.), fVerboseLevel(0)
{
...
/// Main constructor
PndSdsTotChargeConversion::PndSdsTotChargeConversion(Double_t tr, Double_t a, Double_t threshold, Double_t clockfrequency, Int_t VerboseLevel)
  : PndSdsChargeConversion(kToT), fthreshold(0.), Qt(0.), t1e(0.), t2e(0.), ftimestep(0.), fstarttime(0.), fstoptime(0.), ftimeoffset(0.), ftimewalk(0.), fVerboseLevel(0)
{
...
{
  Double_t temp = floor((Double_t)((time) / ftimestep));
...
{ // function called for different events to set different time offsets
  ftimeoffset = gRandom->Uniform(ftimestep);
}

Double_t PndSdsTotChargeConversion::GetPileUpTime(Double_t fcharge)
{ // returns the time the detector is not sensitive for other events
  return (GetParameter("ftr") + fcharge / GetParameter("fa"));
}

Double_t PndSdsTotChargeConversion::DigiValueToCharge(Double_t digivalue)
{ // returns the charge for the given tot value
  ...
  return (-GetParameter("fa") * GetParameter("ftr") + GetParameter("fth") + digivalue * GetParameter("fa")) / 2. +
         sqrt(pow((GetParameter("fa") * GetParameter("ftr") - GetParameter("fth") - digivalue * GetParameter("fa")), 2) / 4. +
              GetParameter("fa") * GetParameter("fth") * GetParameter("ftr"));
}
