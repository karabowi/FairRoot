...
PndSdsNoiseProducer::PndSdsNoiseProducer()
  : PndSdsTask("Charge Noise Producer"), fBranchNameStrip(""), fBranchNamePixel(""), fDigiStripArray(nullptr), fDigiPixelArray(nullptr), fDigiParRect(nullptr),
    fDigiParTrap(nullptr), fDigiParPix(nullptr), fMCPointType(), // Not used???
    fGeoH(nullptr), fPixelIds4(), fPixelIds6(), fPixelIds8(), fPixelIds12(), fStripRectIds(), fStripTrapIds(), fNoiseSpread(0), fThreshold(0), fIonizationEnergy(1.)
{
...
InitStatus PndSdsNoiseProducer::Init()
{
  ...
  if (!ioman) {
    LOG(error) << " PndSdsNoiseProducer::Init: RootManager not instantiated!";
    return kFATAL;
  }
  ...
  if (!fDigiStripArray) {
    LOG(warn) << " PndSdsNoiseProducer::Init: No " << fBranchNameStrip << " array!";
  ...
  if (!fDigiPixelArray) {
    LOG(warn) << " PndSdsNoiseProducer::Init: No " << fBranchNamePixel << " array!";
    std::cout << "    Create a new one." << std::endl;
  ...
  FillSensorLists();
  if (fVerbose > 1) {
    std::cout << "-I- PndSdsNoiseProducer: Registered Sensors: " << fStripRectIds.size() << "xStripRect " << fStripTrapIds.size() << "xStripTrap " << fPixelIds4.size() << "xPixel"
              << std::endl;
  }
  LOG(info) << " PndSdsNoiseProducer: Intialisation successfull";
...
void PndSdsNoiseProducer::FillSensorLists()
{
  ...
  for (int i = 0; i < sensorNames->GetEntries(); i++) {
    ...
    if (volname.Contains("Active")) {
      if (volname.Contains("Rect"))
        fStripRectIds.push_back(i);
      if (volname.Contains("Trap"))
        fStripTrapIds.push_back(i);
      if (volname.Contains("Pixel")) {
        if (volname.Contains("4x1"))
          fPixelIds4.push_back(i);
        if (volname.Contains("6x1"))
          fPixelIds6.push_back(i);
        if (volname.Contains("4x2"))
          fPixelIds8.push_back(i);
        if (volname.Contains("6x2"))
          fPixelIds12.push_back(i);
      }
    }
  }
}

void PndSdsNoiseProducer::DiveDownNode(TGeoNode *)
{ // nodeMother //[R.K.03/2017] unused variable(s)
...
void PndSdsNoiseProducer::Exec(Option_t *)
{
  ...
  Int_t nrCh = 0, rnd = 0, nrFE = 0, sens = 0, nrSensors = 0, fe = 0, chanmax = 0, chan = 0, col = 0, row = 0,
        // iStrip=0,
    chanwhite = 0,
        // iPix=0,
    charge = 0, nNoisyStripRects = 0, nNoisyStripTraps = 0, nNoisyPixels = 0;
  Double_t xfrac = 0.;
  Int_t did;
  ...
  if (fVerbose > 1)
    LOG(info) << " PndSdsNoiseProducer: RECT xfrac = " << xfrac << " leading to " << chanwhite << " noisy digis of " << chanmax << " total channels";
  ...
  chanwhite = gRandom->Poisson(xfrac * chanmax);
  if (fVerbose > 1)
    LOG(info) << " PndSdsNoiseProducer: TRAP xfrac = " << xfrac << " leading to " << chanwhite << " noisy digis of " << chanmax << " total channels";
  for (Int_t i = 0; i < chanwhite; i++) {
    rnd = gRandom->Integer(chanmax);
    sens = rnd / (nrFE * nrCh);
    rnd = rnd % (nrFE * nrCh);
  ...
  chanwhite = gRandom->Poisson(xfrac * chanmax);
  if (fVerbose > 1)
    LOG(info) << " PndSdsNoiseProducer: PIXEL xfrac = " << xfrac << " leading to " << chanwhite << " noisy digis of " << chanmax << " total channels";
  for (Int_t i = 0; i < chanwhite; i++) {
    ...
    if (fe >= (Int_t)(4 * fPixelIds4.size() + 6 * fPixelIds6.size() + 8 * fPixelIds8.size())) {
      ...
      fe = fe % 12;
      if (fe > 6)
        fe = fe - 6 + 10; // 0-9 one row of FE, 10-19 2nd row of FE
      ...
      fe = fe % 8;
      if (fe > 4)
        fe = fe - 4 + 10; // 0-9 one row of FE, 10-19 2nd row of FE
  ...
  // *** The End ***
  if (fVerbose > 0) {
    LOG(info) << " PndSdsNoiseProducer: Noise produced\t" << nNoisyStripRects << "xStripRect\t" << nNoisyStripTraps << "xStripTrap\t" << nNoisyPixels << "xPixels";
  }
}
// -------------------------------------------------------------------------
...
void PndSdsNoiseProducer::AddDigiStrip(Int_t &noisies, Int_t iPoint, Int_t sensorID, Int_t fe, Int_t chan, Double_t charge)
{
  ...
  if (found == kFALSE) {
    ...
    noisies++;
    if (fVerbose > 2)
      std::cout << " -I- PndSdsNoiseProducer: Added StripTrap Digi at: FE=" << fe << ", channel=" << chan << ", charge=" << charge << " e"
                << ", in sensor \n"
                << sensorID << std::endl;
  }
}
...
void PndSdsNoiseProducer::AddDigiPixel(Int_t &noisies, Int_t iPoint, Int_t sensorID, Int_t fe, Int_t col, Int_t row, Double_t charge)
{
  ...
  for (Int_t kstr = 0; kstr < iPix && found == kFALSE; kstr++) {
    ...
    if (aDigi->GetDetID() == detID && aDigi->GetSensorID() == sensorID && aDigi->GetFE() == fe && aDigi->GetPixelColumn() == col && aDigi->GetPixelRow() == row) {
      aDigi->AddCharge(charge);
      aDigi->AddIndex(iPoint);
      found = kTRUE;
    }
  ...
  if (found == kFALSE) {
    ...
    noisies++;
    if (fVerbose > 2)
      std::cout << " -I- PndSdsNoiseProducer: Added Pixel Digi at: FE=" << fe << ", col|row = (" << col << "|" << row << "), charge=" << charge << " e"
                << ", in sensor \n"
                << sensorID << std::endl;
  }
}
